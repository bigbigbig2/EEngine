...
const artifactId = currentBuild.dirty
  ...
  : currentBuild.commit;
const outputRoot = path.join(repoRoot, "temp/r5/fx-06b", artifactId);
const baseUrl = process.env.OENGINE_EXAMPLES_URL ?? "http://127.0.0.1:5174";
const chromePath = process.env.CHROME_PATH ?? "C:/Program Files/Google/Chrome/Application/chrome.exe";
const requireClean = process.env.FX06B_REQUIRE_CLEAN === "1";
const profile = process.env.FX06B_PROFILE === "smoke" ? "smoke" : "full";
const targetGpuDeviceClass = "GeForce RTX 2060 SUPER";
...
const browserSystemInfo = await captureBrowserSystemInfo(browser);
const browserGpuDevices = browserSystemInfo?.gpu?.devices ?? [];
const targetGpuMatched = browserGpuDevices.some((device) =>
  String(device.deviceString ?? "").includes(targetGpuDeviceClass)
);
...
try {
  ...
  await page.goto(`${baseUrl}/r5-final-temporal/?profile=${profile}`, {
  ...
  while (true) {
    await page.waitForFunction(
      (expected) => {
        const state = window.__OENGINE_FX_06B_STATE__;
    ...
    const state = await page.evaluate(() => window.__OENGINE_FX_06B_STATE__);
    if (state?.completed) {
    ...
    await page.evaluate(() => window.__OENGINE_FX_06B_ADVANCE__?.());
    expectedEvent++;
  }
  await context.close();
} finally {
...
const featureComparisons = {
  taaToAo: await compareScreenshots(
    screenshotByKey("taa-only", "end")?.path,
    screenshotByKey("ao-temporal", "end")?.path,
    PNG
  ),
  taaToSsr: await compareScreenshots(
    screenshotByKey("taa-only", "end")?.path,
    screenshotByKey("ssr-temporal", "end")?.path,
    PNG
  ),
  taaToCombined: await compareScreenshots(
    screenshotByKey("taa-only", "end")?.path,
    screenshotByKey("static-b", profile === "smoke" ? "end" : "variance-15")?.path,
    PNG
  )
};
const thresholds = {
...
for (const stageId of ["disocclusion", "transparent-motion"]) {
...
for (const [stageId, label] of (profile === "full" ? [
  ["camera-cut", "first"],
  ["resize", "first"],
  ["resolution-085", "first"],
  ["resolution-067", "first"],
  ["resolution-05", "first"],
  ["resolution-return-1", "first"],
...
for (const [name, comparison] of profile === "full" ? Object.entries(featureComparisons) : []) {
  if (comparison === null) {
    imageIssues.push(`${name}: feature comparison evidence is incomplete`);
  } else if (comparison.rmsRgb8 < thresholds.minimumFeatureDifferenceRmsRgb8) {
    imageIssues.push(`${name}: feature single/combined image is indistinguishable`);
  }
}
if (profile === "full" && (temporalStaticVariance?.rmsRgb8 ?? Number.POSITIVE_INFINITY) > thresholds.maxStaticTemporalRmsRgb8) {
...
if (profile === "full" && (settledDifference?.rmsRgb8 ?? Number.POSITIVE_INFINITY) > thresholds.maxSettledRmsRgb8) {
  imageIssues.push(`settled temporal RMS ${settledDifference.rmsRgb8} shows sustained ghosting`);
}
for (const [stageId, metric] of profile === "full" ? Object.entries(ghostSequences) : []) {
  if (metric === null) {
...
const hardwareIssues = profile !== "full" || targetGpuMatched
  ? []
  : [`performance target GPU ${targetGpuDeviceClass} is not the active browser adapter`];
const issues = [
...
const gate = {
  ...
  gateEligible: profile === "full" && targetGpuMatched &&
    scopedDirtyReasons.length === 0 && build.passed,
  requireClean,
  cleanScope: "OEngine/docs/examples; three.js reference submodule worktree excluded",
  excludedReferenceDirtyReasons,
  performanceTarget: {
    deviceClass: targetGpuDeviceClass,
    matched: targetGpuMatched,
    browserGpuDevices
  },
  issues
};
...
const sequence = {
  ...
  renderScaleTransition: [1, 0.85, 0.67, 0.5, 1],
...
async function captureBrowserSystemInfo(browser) {
  try {
    const session = await browser.newBrowserCDPSession();
    const result = await session.send("SystemInfo.getInfo");
