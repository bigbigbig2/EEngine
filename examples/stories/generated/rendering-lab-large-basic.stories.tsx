// Generated by scripts/generate-stories.mjs. Do not edit.
import type { Meta, StoryObj } from "@storybook/react-vite";

const exampleBaseUrl = (import.meta.env.VITE_EXAMPLE_BASE_URL ?? "http://localhost:5173").replace(/\/$/, "");

function ExampleFrame() {
  return (
    <iframe
      title="Rendering Lab - Large Model Basic"
      src={`${exampleBaseUrl}/demos/14-integrated/rendering-lab-large-basic/`}
      style={{ width: "100%", height: "100vh", border: 0, display: "block" }}
      allow="fullscreen"
    />
  );
}

const meta = {
  title: "Examples/14 Integrated",
  component: ExampleFrame,
  parameters: {
    docs: { description: { story: "Large GLB model with Unlit shading and effects off, automatic bounds fitting, and the shared GPU/CPU performance experiment panel." } },
    tags: ["integrated","large-model","packed-scene","gltf","unlit","effects-off","profiling"]
  }
} satisfies Meta<typeof ExampleFrame>;

export default meta;
type Story = StoryObj<typeof meta>;

export const RenderingLabLargeBasic: Story = { name: "Rendering Lab - Large Model Basic" };
