...
- `gpu-counters` readback 只出现在采样帧，采样完成后 `pending=false`；每个有效样本必须包含 `candidateInstances`、`visibleInstances`、`rejectedFrustum`、`candidateClusters`、`selectedClusters`、`hwClusters`、`alphaClusters`、`hwTriangles`、`shadedPixels`、`emptyVisibilityPixels`、`activeMaterials`、`activeLights` 与 `queueOverflowMask`；
