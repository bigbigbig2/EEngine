...
test("R3-B owner allocates only root/ping-pong/selected resources and encodes GPU rounds", () => {
  ...
  const prepared = generator.prepare(scene, { sseThreshold: 24 });
  ...
  assert.equal(gpu.buffers.length, 9);
  assert.equal(
  ...
  assert.equal(encoder.directDispatches.length, 1);
  assert.deepEqual(encoder.directDispatches[0], [1, 1, 1]);
  assert.equal(encoder.indirectDispatches.length, 3);
  assert.ok(encoder.indirectDispatches.every((dispatch) => dispatch.offset === 0));
  assert.equal(encoder.copies.length, 5, "root + 3 rounds + selected evidence");
