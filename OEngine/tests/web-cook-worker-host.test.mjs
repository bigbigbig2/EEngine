...
test("Dedicated Worker host runs CPU cook session and transfers descriptor/page ownership", async () => {
  ...
  const host = new WebCookWorkerHost({ port, source: { fetch: async (_url, init) => { const match = String(init.headers.Range).match(/bytes=(\d+)-(\d+)/); const start = Number(match[1]), end = Number(match[2]); return new Response(glb.slice(start, end + 1), { status: 206, headers: { "Content-Range": `bytes ${start}-${end}/${glb.byteLength}`, "Content-Encoding": "identity" } }); } }, cooker: { async cookBootstrap() { return { descriptor: encodeGeometryProductDescriptorBinaryV1(product.descriptor), productId: product.productId, revision: 0, pageCount: 1, async readPage(pageId) { return { pageId, decodedHash128: product.descriptor.pageRecords.slice(0, 16), decodedPageHash128: product.descriptor.pageRecords.slice(0, 16), bytes: product.page.buffer }; }, release() {} }; } } });
