...
const { decodeGroupHeaderV3, decodeMeshletHeaderV3, decodeHierarchyNodeV3 } = await import("../.test-dist/assets/GeometryAbiV3.js");
...
function buildGridGlb(size = 33) {
  ...
  const json = {
    ...
    meshes: [{ name: `corpus-${size}`, primitives: [{ attributes: { POSITION: 0, NORMAL: 1, TEXCOORD_0: 2 }, indices: 3, material: 0 }] }],
...
async function cookWithWebRuntime(glb, module) {
  ...
  try {
    ...
    const result = cookWebGeometryWasmV1(module, encodeWebCanonicalGeometryV1(domains), encodeWebGeometryCookRecipeV1(), 64 * 1024 * 1024);
...
function summarize(sections, pages) {
  ...
  let boundsMin = [Infinity, Infinity, Infinity], boundsMax = [-Infinity, -Infinity, -Infinity], maxError = 0, maxMeshletVertices = 0, maxMeshletTriangles = 0;
  ...
  return {
    ...
    boundsMin, boundsMax, maxError, maxMeshletVertices, maxMeshletTriangles
  };
}
