...
export const GPU_COUNTER_FIELDS = [
  ...
  { name: "temporalReactivePixels", index: 69, semantic: "sampled TAA output or NSS internal positions whose closest-surface/current reactive signal reaches the configured diagnostic threshold" },
  { name: "temporalDisoccludedPixels", index: 70, semantic: "sampled TAA output or NSS internal positions whose closest-surface confidence is below the configured diagnostic threshold" },
  { name: "temporalHistoryRejectedPixels", index: 71, semantic: "sampled TAA output positions rejected by production hard gates or NSS internal positions whose packed history validity rounds to zero" },
  ...
  { name: "setupOverflow", index: 102, semantic: "sampled optional LargeTriangleSetup records falling back because the dense cache bound was exceeded" },
  ...
  { name: "geometryRiskyTriangles", index: 108, semantic: "ADR-0008 triangles in meshlets exclusively routed through selective exact raster" },
