...
export class GeometryPageSchedulerV1 {
  ...
  registerProduct(productTableSlot: number, generation: number, source: GeometryProductRevisionSourceV1, options: GeometryPageRegistrationOptionsV1 = {}): void { const pageCount = source.descriptor.pageRecords.byteLength / GEOMETRY_PRODUCT_PAGE_RECORD_STRIDE, pageBytes = source.descriptor.decodedPageBytes; if (!Number.isInteger(productTableSlot) || productTableSlot < 0 || !Number.isInteger(generation) || generation <= 0 || generation === 0xffffffff || !Number.isInteger(pageCount) || pageCount <= 0 || !Number.isInteger(pageBytes) || pageBytes <= 0 || pageBytes > this.#options.maxInFlightBytes || pageBytes > this.#options.maxUploadBytesPerFrame) throw new RangeError("invalid Geometry Product scheduler registration or page budget"); if (this.#products.has(generation)) throw new Error(`product generation ${generation} is already registered`); if (options.sourceOwnership !== undefined && options.sourceOwnership !== "scheduler" && options.sourceOwnership !== "external") throw new RangeError("invalid Geometry Product source ownership"); this.#products.set(generation, { productTableSlot, generation, source, pageCount, ownsSource: options.sourceOwnership !== "external" }); }
  ...
  tick(nowMs = 0): void {
    if (!Number.isFinite(nowMs) || nowMs < 0) throw new RangeError("Geometry page scheduler time must be non-negative");
    this.pump(nowMs);
  }
  ...
  private async produce(operation: Operation, nowMs: number): Promise<void> { try { const page = await operation.product.source.readPage(operation.demand.pageId, operation.controller?.signal); const descriptor = operation.product.source.descriptor; const expected = decodeGeometryProductPageRecordV1(descriptor, operation.demand.pageId); if (page.revision !== descriptor.revision || page.pageId !== operation.demand.pageId || !sameBytes(page.productId, descriptor.productId) || page.bytes.byteLength !== descriptor.decodedPageBytes || !sameBytes(page.decodedHash128, expected.decodedHash128)) throw new Error("page key/size/identity mismatch"); const digest = new Uint8Array(await globalThis.crypto.subtle.digest("SHA-256", page.bytes.slice(0))); if (!sameBytes(digest.subarray(0, 16), page.decodedPageHash128)) throw new Error("page integrity hash mismatch"); if (this.#products.get(operation.product.generation) !== operation.product || operation.controller?.signal.aborted) { this.#lateResults++; operation.state = "failed"; this.#failed++; return; } operation.page = Object.freeze({ ...page, productId: page.productId.slice(), decodedHash128: page.decodedHash128.slice(), decodedPageHash128: page.decodedPageHash128.slice(), bytes: page.bytes.slice(0) }); operation.state = "verified"; operation.state = "upload-queued"; } catch (error) { if (this.#products.get(operation.product.generation) !== operation.product || operation.controller?.signal.aborted) { this.#lateResults++; operation.state = "failed"; return; } operation.attempts++; const deterministic = /hash|size|key|profile|corrupt|unsupported|identity|integrity/i.test(error instanceof Error ? error.message : String(error)); if (!deterministic && operation.attempts <= this.#options.maxRetries) { operation.state = "queued"; operation.nextRetryAt = nowMs + this.#options.retryBaseDelayMs * (2 ** (operation.attempts - 1)); this.#retries++; } else { operation.state = "failed"; this.#failed++; } } }
}
