...
interface RegisteredProduct { readonly generation: number; readonly productTableSlot: number; readonly source: GeometryProductRevisionSourceV1; readonly pageCount: number; }
...
export class GeometryPageSchedulerV1 {
  ...
  registerProduct(productTableSlot: number, generation: number, source: GeometryProductRevisionSourceV1): void { if (!Number.isInteger(productTableSlot) || productTableSlot < 0 || !Number.isInteger(generation) || generation <= 0 || generation === 0xffffffff) throw new RangeError("invalid Geometry Product scheduler registration"); if (this.#products.has(generation)) throw new Error(`product generation ${generation} is already registered`); this.#products.set(generation, { productTableSlot, generation, source, pageCount: source.descriptor.pageRecords.byteLength / GEOMETRY_PRODUCT_PAGE_RECORD_STRIDE }); }
  unregisterProduct(generation: number): void { const product = this.#products.get(generation); if (!product) return; for (const [key, operation] of this.#operations) if (operation.product.generation === generation && operation.state !== "resident") { operation.controller?.abort(); operation.state = "failed"; this.#operations.delete(key); this.#cancelled++; } this.#products.delete(generation); product.source.release(); }
