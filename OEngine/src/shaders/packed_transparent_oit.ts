...
@group(0) @binding(16) var oengine_texture_bank_1: texture_2d_array<f32>;
...
fn packed_transparent_vertex(
  ...
  let world = oengine_instance_current_object_to_world(instance) * vec4f(local_position, 1.0);
...
fn transparent_material(input: TransparentVertexOutput) -> OEngineMaterialVisibilityRecord {
  return materials[min(input.material_handle, arrayLength(&materials) - 1u)].payload;
}
...
fn validate_transparent_fragment(
  ...
  return !oengine_texture_ref_valid(material.texture_ref) ||
