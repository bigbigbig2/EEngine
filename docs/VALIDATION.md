...
命中的真实浏览器 Case 继续以 registry id 运行；`shading-bin-component` 是 ADR-0013 Step 4 的 deterministic producer/readback Case，`shading-resolve-component` 是 Step 5 的 16-program/mixed-set/numeric/fault/diagnostics consumer Case；两者都是 component isolation。`sparse-shading-candidate` 当前拥有 Step 6 `correctness-v6` 的 L4 垂直切片：`MixedBins` 运行真实 Hardware Visibility 双 MRT、classifier/finalizer、16 个 GPU 间接 specialized consumer、direct lighting、Final Output 与异步 capture；`BasicCubeNear/Far` 在同一 immutable publication 上只更新 reverse-Z 透视相机，逐像素验证解析覆盖、背景 sentinel/zero HDR、microtile queue、indirect args、coverage slope 和 unlit feature-off topology；`UnlitVertexColor` 由 VisibilityKey primitive 驱动独立的 CPU 重心插值 HDR oracle；`UnlitTexture` 对明确 TextureBindingSet 的非均匀 4×4 texture 复现 UV 重建、nearest-clamp sampling 和材质因子；`RenderingLabFixed` 以四个 program 的 lit/unlit/textured 几何、真实 GPU shadow raster、IBL、production HZB/GI/SSGI/SSR/temporal/TAA/Tonemap owner 组成同一 28-pass FrameGraph，并以 cold/warm/stable 三帧检查提交后历史推进和稳定数值输出；`LifecycleResize` 在 256×256 旧帧仍 in-flight 时原子发布 256×128 的新 heap/args/layout 与全部 snapshot-dependent GPU records，证明旧 GPU revision 只在 submission completion 后退役，并在 resized/reuse 两帧继续通过逐像素与 queue oracle。十一个采样帧各自只有一次 main submit，private submit 为零。该 Case 仍不覆盖 material/association patch、TextureBindingSet relocation、feature toggle、scene replace、camera cut、aborted submit、device-loss recovery、production cutover 或 PERF。`protocol-self-test` 与 `webgpu-component` 只是宿主编排和通用 WebGPU smoke，不能替代具体 Renderer/算法的 MILESTONE 或 PERF。
