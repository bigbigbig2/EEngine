...
命中的真实浏览器 Case 继续以 registry id 运行；`shading-bin-component` 是 ADR-0013 Step 4 的 deterministic producer/readback Case，`shading-resolve-component` 是 Step 5 的 16-program/mixed-set/numeric/fault/diagnostics consumer Case；两者都是 component isolation。`sparse-shading-candidate` 当前拥有 Step 6 `correctness-v4` 的 256×256 L4 垂直切片：`MixedBins` 运行真实 Hardware Visibility 双 MRT、classifier/finalizer、16 个 GPU 间接 specialized consumer、direct lighting、Final Output 与异步 capture；`BasicCubeNear/Far` 在同一 immutable publication 上只更新 reverse-Z 透视相机，逐像素验证解析覆盖、背景 sentinel/zero HDR、microtile queue、indirect args、coverage slope 和 unlit feature-off topology；`UnlitVertexColor` 由 VisibilityKey primitive 驱动独立的 CPU 重心插值 HDR oracle；`UnlitTexture` 对明确 TextureBindingSet 的非均匀 4×4 texture 复现 UV 重建、nearest-clamp sampling 和材质因子。五个采样帧各自只有一次 main submit。该 Case 仍不覆盖 RenderingLabFixed、完整 lifecycle matrix、production cutover 或 PERF。`protocol-self-test` 与 `webgpu-component` 只是宿主编排和通用 WebGPU smoke，不能替代具体 Renderer/算法的 MILESTONE 或 PERF。
