...
- Verification: `OEngine/npm test` 440/440 passed. The clean three-session formal backend A/B is now recorded below; legacy comparison and identity-bearing pixel parity remain before closing the full M3 release Gate.
- Clean formal rerun: class-depth `temp/visibility-to-surface/2c09e8eb-0a9a-44ab-ac1a-4a258cd7446f/report.json` and class-discard `temp/visibility-to-surface/361d9a01-4563-41c8-bc02-1c660ce80e1e/report.json` each contain 3 independent sessions at 1920x1080/DPR1 with 120+480 cadence and empty provenance/browser/GPU gate diagnostics. All six screenshots show the restored red cube. Resolve P50 samples were class-depth 2.370464/2.579264/2.535376 ms and class-discard 2.476624/2.510848/2.462496 ms.
- Gate boundary: this closes the previously observed class-depth black-screen regression and provides clean backend evidence, but it is not the full M3 release Gate yet: the runner still needs identity-bearing per-attachment pixel parity against the legacy baseline and the historical `legacy-pixel-queue` clean comparison. M3 remains `implemented-awaiting-evidence` for release acceptance.
