...
- Packed hierarchy/work generation、Hardware Visibility、直接 `VisibilityKey`、MaterialTileWork compute evaluation 和统一 Surface ABI bridge 已有生产 owner。
...
- Surface：`MaterialTileWork`、compute material evaluator、`GpuSurfaceAbi` transition bridge 与 `SurfaceFeature`。
...
- MaterialTileWork 已形成 GPU classifier → compute material evaluator → compute direct-lighting consumer 闭环，full material evaluation 只发生一次；Surface V1 现在只由无材质逻辑的格式 bridge 生成。Step 3 仍必须把 GI/AO/SSR/Temporal/debug consumers 迁到 `ShadingSurfaceLite`/conditional `DiffuseSurfaceLite`/明确 HDR products，删除 bridge 与一次性 albedo/emissive/full-ORM attachment，并用同条件综合 benchmark 证明最终 bytes/pixel 与 bandwidth；第二 GPU vendor 与 clean PERF 证据仍缺失。
...
2. [ADR-0009](./adr/0009-compute-shading-and-advanced-frame-pipeline-v2.md)：Step 0–2 已完成；下一项执行 Step 3 SurfaceLite/HDR consumer cutover，删除纯格式 bridge 和旧 attachment，并冻结 consumer-driven companion products 与最终物理 profile。
