...
| Web Runtime Cooker 主路线 | in progress | 已加入严格 206/有预算 200 fallback 的 GLB Range source、按 accessor 精确 Range 的 compact scene catalog/cook units、带 source/WASM/output budget 与取消/credit 的 `WebCookCoordinator`、generation-filtered Dedicated Worker transport，以及把 live descriptor/page 事件映射到 producer-neutral revision source 的有界 Web Product provider；尚无 Nyx WASM cooker 与 worker entry | 完成 Nyx Worker/WASM bootstrap Product -> 同一生产像素路径；不得把 coordinator/source 视为 cooker 完成 |
