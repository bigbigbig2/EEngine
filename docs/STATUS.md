...
On revision `fd33257` this case is `accepted` (clean tree, Chrome 153 headless):
the activation cut is 24 of 53 pages and fully resident, the richer revision
lands with `replacements 1`, GPU demand reaches the scheduler with
`requested 330` / `failed 0` and moves `residentPages` 374 to 375, coverage holds
at 17394 lit pixels, and `recoverableFailures` stays 0.
