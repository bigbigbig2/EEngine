...
| Web Runtime Cooker 主路线 | in progress | 已加入严格 206/有预算 200 fallback 的 GLB Range source、按 accessor 精确 Range 的 compact scene catalog/cook units，以及带 source/WASM/output budget、取消、generation 和 output credit 的 `WebCookCoordinator`；尚无 Nyx WASM cooker、Dedicated Worker transport 或 Web Product provider | 完成 Nyx Worker/WASM bootstrap Product -> 同一生产像素路径；不得把 coordinator/source 视为 cooker 完成 |
