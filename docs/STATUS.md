...
- ADR-0009 Step 4 已完成 Three.js r186-derived GTAO production replacement并通过MILESTONE：唯一 `mode=gtao` owner保留pinned `interleavedGradientNoise + rand`、magic-square/temporal rotation、quadratic horizon integration、physical radius/thickness/falloff，并从同一trace输出AO与bent normal；temporal-only输入和feature-off资源均可物理裁剪。Chrome `surface.gtao-replacement` 11/11断言通过，half-resolution 3×6×2=36 depth samples/pixel、四阶段各一次、one-submit与WebGPU diagnostics零错误成立；旧step4 PERF只作历史对照，当前实现等待统一final PERF与扩展薄几何/运动视觉复核。
...
- ADR-0009 Step 6 已完成Three.js r186-derived SSR replacement并通过MILESTONE：production链为bounded GGX VNDF/HZB trace→receiver-resolved hit shading→双候选TemporalReproject→8-tap Vogel RecurrentDenoise→conditional upscale→baseline correction；旧48-neighbor/fallback/二次BRDF路径已删除。Chrome验证修复了correction shader误用partial Surface snippet；`surface.ssr-replacement` 11/11通过，127,029 trace pixels、26,252 hits、2,401,880 trace steps、max 128，temporal-off和feature-off裁剪、one-submit及diagnostics零错误成立。镜面fixture截图可辨，但长时间稳定性、roughness/edge矩阵与性能仍待扩展视觉/final PERF。
...
- ADR-0009 Step 4–10当前MILESTONE汇总：`OEngine npm test` 127/127，examples验证工具16/16与生产build通过；六个Surface Chrome cases共70项scenario断言全部通过且浏览器错误为零。1920×1080/DPR1 `comprehensive-full` pipeline matrix执行base/full及九个`full-minus-*`，每路60 measured frames均为main-submit P50=1、stable invalid frames=0、overflow maximum=0。dirty-tree DEV profile的visible base/full GPU P50约1.261/2.653 ms、CPU P50 4.4/12.8 ms，只证明编排和风险，不是正式性能声明。
