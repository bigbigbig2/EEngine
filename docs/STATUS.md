...
- ADR-0009 Step 4 已完成：`mode=gtao` 的唯一 production AO 已替换为 pinned three.js r186 `GTAONode` local WGSL port，保留 3/5 direction、quadratic stepping、magic-square/temporal jitter、view-space thickness、squared falloff 与 horizon integral；同一 trace 扩展 world-space bent normal，不存在第二个旧 AO 或 Three.js TRAA owner。AO/second moment/oct bent normal 以 `rgba16float` 共同做 spatial/temporal filter，最终 joint resolve 拆成 `r8unorm + rg16uint`。真实 Chrome `surface.gtao-replacement` 证明 production graph 单 trace、两张 history、one-submit、薄几何/接触边缘观察与 feature-off 的 pass/resource/history/counter 全裁剪，GPU diagnostics 为零。clean PERF 保存在 `OEngine/benchmarks/advanced-frame-pipeline-v2-step4.json`：三个独立 context 的 518,400 GTAO pixels 在所有 sampled frame 中 history accepted 全量、rejected 为 0；GPU frame P50/P95 为 1.962/2.284 ms。相对 Step 3，完整 GTAO phase P50 增加 0.089 ms（15.90%），AO history 增加 4,147,200 B，joint resolve P50 降低 15.16%、transient resource 少 1；这是已接受的上游质量/厚度/时域 bent-normal replacement 成本，不声明性能提升。
- ADR-0009 Step 5 已部分落地但尚未完成：公开配置已切成单值 `screenSpaceDiffuseMode = off | gtao | ssgi`；新增 pinned three.js r186 SSGI horizon-bitfield trace、AO/GI/bent/confidence joint spatial/temporal resolve、独立四纹理 history owner、pre-SSGI radiance source 与 `ScreenSpaceDiffuseResolve`。production long-range GI 已改由单个 `LongRangeDiffuseProviderPass` 逐 receiver 早返回选择 `valid Brick4 > valid Probe Volume > IBL > black`，不会运行三套 fullscreen candidate；输出唯一 provider identity、diffuse irradiance、baseline specular radiance，并由 schema v21 统计 provider/invalid-generation/nonresident/unassigned/duplicate。Brick4/LPV owner 具备 generation/availability，但 Brick4 receiver mapping generation 与 required-brick residency 尚未进入资产/receiver ABI；旧 `indirect_lighting_mode` plumbing 和不可达 legacy pass 分支也尚待 Step 10 删除。按本轮“不跑代码测试”约束，类型/构建、真实 Chrome `surface.ssgi-production`、feature matrix、视觉检查与唯一 `comprehensive-full` PERF 均尚未执行，因此 Step 5 仍是 partial-implementation/evidence-open，不能写“已完成”。
...
- MaterialTileWork 已形成 GPU classifier → compute material evaluator → compute direct-lighting consumer 闭环，full material evaluation 只发生一次；生产 consumer、公开接口与 ABI 权威已没有 Surface V1 依赖。Step 5 partial implementation 让 `DiffuseSurfaceLite` 逻辑别名、pre-SSGI source 与 component MRT 只在 `mode=ssgi` 被请求，并已把 receiver-level GI precedence 收敛到一个早返回 producer；Brick4 细粒度 mapping generation/residency、legacy plumbing 删除和 Chrome topology/perf 证据仍缺。第二 GPU vendor 证据仍缺失。
...
2. [ADR-0009](./adr/0009-compute-shading-and-advanced-frame-pipeline-v2.md)：Step 0–4 已完成、Step 5 部分落地；下一项补齐 Brick4 receiver mapping generation/required-brick residency ABI并删除 scene-wide GI exclusive plumbing/不可达 legacy pass，再在允许执行验证后完成类型/构建、`surface.ssgi-production`、`off/gtao/ssgi × SSR off/on` topology matrix 与唯一 `comprehensive-full` clean PERF，最后决定是否关闭 Step 5。
