...
Step 7 的静态收敛已完成：生产代码只保留一个 Render World、VisibilityKey attachment、Surface/velocity producer、MBOIT、directional CSM consumer 和 graph recipe；旧 runtime、Pass、shader/layout/pipeline/counter、双 ID attachment及公开 owner evidence 字段均已删除。shader audit 无 dead/unknown authored source。正式 clean-commit 性能 artifact 完成后关闭本步骤。
