...
- MaterialTileWork 已形成 GPU classifier → compute material evaluator → compute direct-lighting consumer 闭环，full material evaluation 只发生一次；生产 consumer、公开接口与 ABI 权威已没有 Surface V1 依赖。IBL/LPV/Brick4 的 SSR-off baseline-specular 中间已物理融合/裁剪，Step 3 clean comprehensive PERF 已关闭。`DiffuseSurfaceLite` 与 pre-SSGI radiance source 要在 Step 5 SSGI consumer 接入时按需创建，当前不得提前常驻。第二 GPU vendor 证据仍缺失。
...
2. [ADR-0009](./adr/0009-compute-shading-and-advanced-frame-pipeline-v2.md)：Step 0–4 已完成；下一项执行 Step 5 Three.js-derived SSGI integration，保持 `mode=gtao|ssgi` 互斥、long-range provider receiver-validity selection、pre-SSGI radiance source 无 self-feedback，以及 SSGI-off 的 `DiffuseSurfaceLite`/history 物理裁剪。
