...
- ADR-0009 Step 5 实现已落地但验证尚未关闭：公开配置使用单值 `screenSpaceDiffuseMode = off | gtao | ssgi`；pinned three.js r186 SSGI、AO/GI/bent/confidence joint resolve、四纹理 history、pre-SSGI source 与能量正确 resolve 已接入。单个 `LongRangeDiffuseProviderPass` 逐 receiver 早返回选择 `valid Brick4 > valid Probe Volume > IBL > black`，不会运行三套 fullscreen candidate；输出唯一 provider identity、selected diffuse irradiance、selected specular radiance，并由 schema v21 统计 provider/invalid-generation/nonresident/unassigned/duplicate。Brick4 V1 完整 tree/probe generation 原子驻留，公开 upload/invalidate 与 immutable-buffer deferred retirement 使 stale generation 只回退、不采样。scene-wide `indirect_lighting_mode`/`ShadeIndirectLightingMode`、topology bit、旧 Brick4/LPV/IBL pass/shader 已全部删除。SSR-only 现在由专用两 MRT resolve 物化 BRDF-weighted/occluded baseline，raw provider radiance 与 receiver-resolved baseline 在 ABI 中分字段，不会错误 subtract；SSGI 才写第三个 diffuse component。按“不跑代码测试”约束，类型/构建、真实 Chrome 两阶段 Brick4 oracle、feature matrix、视觉检查与唯一 `comprehensive-full` PERF 均未执行，所以 Step 5 是 implementation-landed/verification-open，不能写“完成”。
...
2. [ADR-0009](./adr/0009-compute-shading-and-advanced-frame-pipeline-v2.md)：Step 0–4 已完成、Step 5 implementation-landed/verification-open；下一项在允许执行验证后完成类型/构建、`surface.ssgi-production` 两阶段 Brick4 generation oracle、`off/gtao/ssgi × SSR off/on` topology matrix、视觉检查与唯一 `comprehensive-full` clean PERF，关闭 Step 5 后进入 Step 6 Three.js-derived SSR replacement。
