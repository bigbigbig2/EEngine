...
- ADR-0009 Step 5 已部分落地但尚未完成：公开配置已切成单值 `screenSpaceDiffuseMode = off | gtao | ssgi`；新增 pinned three.js r186 SSGI horizon-bitfield trace、AO/GI/bent/confidence joint spatial/temporal resolve、独立四纹理 history owner、pre-SSGI radiance source 与 `ScreenSpaceDiffuseResolve`。Brick4/Probe alpha 已表达 receiver validity，所选 producer 的 invalid receiver 会在同一 resolve 回退 IBL；但当前仍由 scene/frame 级 `indirect_lighting_mode` 选择 Brick4 或 Probe，尚未实现同帧 `valid Brick4 > valid Probe Volume > IBL` 的完整 receiver-level chain。SSGI topology 才物化 long-range diffuse/specular component，GTAO 与 SSGI owner 互斥。按本轮“不跑代码测试”约束，新增静态断言、类型/构建、真实 Chrome `surface.ssgi-production`、feature matrix 与唯一 `comprehensive-full` PERF 均尚未执行，因此 Step 5 是 partial-implementation/evidence-open，不能写“已完成”。
...
- MaterialTileWork 已形成 GPU classifier → compute material evaluator → compute direct-lighting consumer 闭环，full material evaluation 只发生一次；生产 consumer、公开接口与 ABI 权威已没有 Surface V1 依赖。IBL/LPV/Brick4 的 SSR-off baseline-specular 中间已物理融合/裁剪，Step 3 clean comprehensive PERF 已关闭。Step 5 partial implementation 让 `DiffuseSurfaceLite` 逻辑别名、pre-SSGI source 与 component MRT 只在 `mode=ssgi` 被请求；完整 receiver-level GI precedence 仍缺，物理裁剪仍待 Chrome topology matrix 证明。第二 GPU vendor 证据仍缺失。
...
2. [ADR-0009](./adr/0009-compute-shading-and-advanced-frame-pipeline-v2.md)：Step 0–4 已完成、Step 5 部分落地；下一项先删除 scene-wide GI exclusive plumbing，完成不运行多个 fullscreen provider 的 receiver-level `Brick4 > Probe Volume > IBL` 选择，再在允许执行验证后完成类型/构建、`surface.ssgi-production`、`off/gtao/ssgi × SSR off/on` topology matrix 与唯一 `comprehensive-full` clean PERF，最后决定是否关闭 Step 5。
