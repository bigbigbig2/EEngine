...
2. [ADR-0009](./adr/0009-compute-shading-and-advanced-frame-pipeline-v2.md)：Step 0–4 已完成、Step 5 implementation-landed/verification-open；下一项按 Step 10 删除 scene-wide GI exclusive plumbing/不可达 legacy pass，并在允许执行验证后完成类型/构建、`surface.ssgi-production` 两阶段 Brick4 generation oracle、`off/gtao/ssgi × SSR off/on` topology matrix、视觉检查与唯一 `comprehensive-full` clean PERF，再决定是否关闭 Step 5 并进入 Step 6。
