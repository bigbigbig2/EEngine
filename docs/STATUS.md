...
| Web Runtime Cooker 主路线 | in progress | 已加入严格 206/有预算 200 fallback 的 GLB Range source、按 accessor 精确 Range 的 compact scene catalog/cook units、带 source/WASM/output budget 与取消/credit 的 `WebCookCoordinator`、generation-filtered Dedicated Worker transport、live Product provider，以及 container-neutral decoded Product assembly；browser-first target 已以 Nyx geometry-builder C++ producer 建立 canonical/recipe/result ABI、逐页 output-credit copy wrapper 和 native determinism/negative oracle，但尚无真实 Emscripten build evidence，也没有 GLB accessor canonicalizer/Worker entry | 完成真实 Emscripten build 与 Worker bootstrap Product -> 同一生产像素路径；不得把 native ABI oracle 视为 S2 Runtime 完成 |
