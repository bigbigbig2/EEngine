...
- 多 session 压力回归覆盖 4 个 client、2 个 active lease、2 个等待者、优先级让位和取消/处置竞态；期间 source/WASM/output 峰值分别保持在 4096 / 8192 / 512 KiB 上限内，所有 client 结束后三类 reservation 与 active/waiting 计数归零。该测试还捕获并修复了“dispose 与 waiter grant 同时发生导致 active lease 泄漏”的真实 bug。
...
- `isolated-pthreads` 现在有显式 capability gate：只有 `crossOriginIsolated && SharedArrayBuffer` 才选择 pthread artifact；否则实际选择 `portable-single` 且报告 `cross-origin-isolation-required` 或 `shared-array-buffer-unavailable`，不伪装成 pthread 完成。validation Vite host 已提供 COOP/COEP headers，`glb-web-product-isolated-pthreads` authored-texture smoke 已在 clean commit 上 accepted，证明目标 Worker/WASM artifact 在隔离宿主可握手启动（仍非性能结论）。

本阶段保留的边界：source reservation 以当前 `GlbRangeSource` session 的 source identity/byteLength 作为 owner 生命周期，WASM reservation 以 session configured max 为保守上限；后续仍可在真实多 asset workload 中补 Range cache/canonical input/WASM committed-peak 的细粒度 counter。`portable-pool`/single 的 Node targeted 证据、4-session budget pressure、portable-pool authored-texture browser smoke 与 isolated-pthreads deployment smoke 已齐；浏览器多 asset PERF 和正式对比基线仍属于后续规模化验证，不阻塞第四步 correctness/ownership 退出。
