...
- Shader source-of-truth 静态审计已覆盖 69 个文件，逐项记录 direct/runtime consumer、最近 pipeline owner、generator candidate 与删除候选；FX-02 迁移 Lighting oracle 后当前结论为 59 个 authored-live、5 个 dead candidate、5 个运行中的 oracle/generated ownership blocker。清单见 `OEngine/benchmarks/shader-source-audit.json` 与 `docs/SHADER-SOURCES.md`。
