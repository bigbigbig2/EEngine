...
device loss（`virtual-product-device-loss`）：在 steady-state（richer revision 已接管）手动 `GPUDevice.destroy()` 后 `Renderer.recoverAfterDeviceLoss()` 会重新协商 adapter/device（`adapters 2`、device 不同）、拒绕丢失 Renderer 的提交，并从保留的 Product source 重建整个场景；256x256 linear-HDR 覆盖率在丢失前 5640、恢复后 5610 lit pixels，`selectedClusters 474`、`visibleInstances 798`、0 GPU error。
