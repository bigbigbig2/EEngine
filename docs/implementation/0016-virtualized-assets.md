...
当前状态（2026-09-18）：已用 ADR-0014 宿主在 `glb-web-product` 上跑通浏览器闭环。相机靠近触发 desired page 缺失后，实测 `scheduler.requested 1809` / `deduplicated 68` / upload 1 page，residency `residentPages 374 -> 375`，延迟 readback `submitted 20` / `overflow 4`（有界），ancestor fallback 保持画面（256x256 窗口 17394 lit pixels）且 0 GPU error。尚未跑相机 cut / source failure / upload budget 饱和的专门变体。
