...
当前已补齐第一批：Producer-neutral Product TS 类型/validator、OEGPACK adapter、Product-aware bootstrap/page heap/location table、GLB Range source、按 accessor 精确 Range 的 compact scene catalog/cook units、带 source/WASM/output budget 与取消/credit 的 `WebCookCoordinator`、generation-filtered Dedicated Worker transport、live Product provider、versioned CookSession credit seam、PageDemand ABI 与 Product GPU location mirror，以及 Native/Web 共同消费的 `DecodedGeometryProductV1`、browser-first WASM canonical/recipe/result ABI、Nyx geometry-builder C++ target、native determinism/negative oracle 与 CPU/WASM Worker host。真实 Emscripten 6.0.9 module/wasm artifact 已入库并经 `createDefaultWebCookWorker`/`WebCookWorkerEntrypoint` 接入 Dedicated Worker。S2/S3 又交付：`createWebCookSceneSource` + `Renderer.uploadWebCookedScene` 运行时 facade、`CoalescedRangeReader` Range 合并、progressive bootstrap + richer revision（含原子 release→re-stage→retire 替换）、per-domain 并行 cook（pthread 变体已构建、pool 握手未闭环）、page-global `WebCookBudgetLedger`、按 Product 总页数预分配 bank（修复 demand 上传新建未绑定 bank 导致的黑屏）。`glb-web-product`（真实 Dungeon GLB）已在当前 revision 的 Chrome `accepted`，带 256x256 HDR 覆盖率断言；`tests/nyx-differential-corpus.test.mjs` 提供 Native Offline ↔ Web 结构化 differential 与不变量/负例 corpus。S4/S5 又交付：`glb-web-product` 的真实 GPU demand 闭环（2121 requested / 374→375 resident / ancestor fallback 17394 lit pixels）、`virtual-product-replacement`（accepted；原子换版 + demand 细化 382→383 + 跨提交边界 evict 4 页）与 `virtual-product-device-loss`（accepted；新 device 重建，该 revision 实测 5640→5610 lit pixels；`a6b730a` 上为 12418→12418）。S6 又交付：Offline 第二路线公共入口（`load_oegpack_product` + `Renderer.uploadOegPackScene`）与 `scene.oescene` 冻结 spec，两条 producer 路线共用同一 admission/residency/Visibility 路径与 Scene source 构建器，并删除了 OEGPACK 专用 bootstrap residency adapter。仍缺 `isolated-pthreads`/`portable-pool`、§18.4 的 Native Nyx 参考二进制、Offline/Web 同 workload 的 loading/GPU 对比证据，以及所有 consumer 的 V2 cutover。
...
当前状态（2026-09-18）：已用 ADR-0014 宿主在 `glb-web-product` 上跑通浏览器闭环。相机靠近触发 desired page 缺失后，实测 `scheduler.requested 2121` / `deduplicated 80` / upload 1 page，residency `residentPages 374 -> 375`，延迟 readback `submitted 20` / `overflow 4`（有界），ancestor fallback 保持画面（256x256 窗口 17394 lit pixels）且 0 GPU error。尚未跑相机 cut / source failure / upload budget 饱和的专门变体。
...
当前状态（2026-09-18）：三段浏览器证据均已在 ADR-0014 宿主上 accepted。replacement 与 eviction（`virtual-product-replacement`）：richer CookSession revision 原子替换 bootstrap（`replacements 1`、generation 1→2、revision 0→1、换版全程 5600+ lit pixels），active revision 在真实 GPU demand 下细化（382→383），随后 4 个非 pinned 候选页跨 `onSubmittedWorkDone` 边界驱逐到 379 页（`evictedPages 4`），驱逐后 17400 lit pixels、0 GPU error。

device loss（`virtual-product-device-loss`）：在 steady-state（richer revision 已接管）手动 `GPUDevice.destroy()` 后 `Renderer.recoverAfterDeviceLoss()` 会重新协商 adapter/device（`adapters 2`、device 不同）、拒绕丢失 Renderer 的提交，并从保留的 Product source 重建整个场景；256x256 linear-HDR 覆盖率在该 revision 为丢失前 5640、恢复后 5610 lit pixels（`a6b730a` 上为 12418→12418），`selectedClusters 474`、`visibleInstances 798`、0 GPU error。

这条腿一共修了三个真 bug：(1) `GpuRenderWorld.recoveryScenes()` 把 Product 场景当 packed 场景返回（`geometries: []`），在 `SceneResidencyManifest` 抛 `requires geometry packages`，使恢复在进入 product 循环前就中止；(2) `WebCookCoordinator.requestPages` 丢弃全部 `activationPageIds` 请求，导致 `abandonForDeviceLoss` 释放 GPU bank 后 activation cut 无法重读——现在改为 cut 完整流出后即可重发，并由 provider 丢弃无人等待的重复副本，不占用后续 demand/recovery 所需的 output credit；(3) `restoreDeviceRecovery` 重建 residency 后漏调 `activatePublication()`，product table record 保持 inactive → GPU 遍历 `candidateClusters 0` → 完全黑帧（residency 实际已完整）。

上述语义已由 `tests/web-cook-activation-reserve.test.mjs` headless 锁定：已流出 activation 页重读恰好投递一次且 credit 守恒、未请求的页不得重发、已消费的页可重读且不留下缓冲、被请求的页在窗口被另一 revision 占满时仍能送达、已消费页的无等待重复副本被丢弃。

### S6 · Offline 第二路线 production parity

当前状态（2026-09-18）：已 accepted。`load_oegpack_product` 从 HTTP Range 或内存两种 selection 打开预煮 `.oegpack`，`Renderer.uploadOegPackScene` 把它送入与 Web 路线完全相同的 admission/residency/streaming/Visibility 路径；生产者分叉只剩 provider 与 scene mapper，两者共用 `Renderer.uploadProductScene` facade 与 `buildVirtualGeometrySceneSourceV1` 构建器。`scene.oescene` 合同已入 `docs/specs/oegpack-scene-manifest-v3.md`（严格解析 + 负例矩阵），OEGPACK 专用 bootstrap residency adapter 已删除，bootstrap cut 改走共享 `VirtualGeometryResidency`。

证据：range 与 memory 得到同一 Product identity、同一 activation cut 与同一 GPU topology 计数器，两者在 256x256 linear-HDR 探针下均为 64105 lit pixels；缺失 pack 显式报 “server did not honor byte range (404)” 且不发布任何 residency；A→B 替换切换 Product identity 且画面连续（64105 → 64102）；GPU demand 把 residency 从 1 细化到 5 页（`requested 21`、`deduplicated 9`、`failed 0`、`retries 0`），近距相机下 65536/65536 lit pixels。

尚未做：同 workload 的 Web cook / cold OEG Range / warm HTTP cache loading、residency 与 GPU 时间对比（PERF 级证据）。
