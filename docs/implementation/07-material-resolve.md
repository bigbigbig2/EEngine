...
- 2026-08-30 lifecycle/material-contract 修正加入 texture refcount/free-list/completion-safe retirement、evidence schema v4、separate occlusion explicit reject、`normalTexture.scale` 与真实 unlit Resolve。随后在 clean commit `74e61c02f3fd66d30bafbf02d8b2472305c9347e` 重采 B/C full Gate：Chrome 151、Intel Gen9、`1280×720`、DPR 1、60 warm-up + 180 sample、每 6 帧 timestamp/counter；两组均 `passed=true`、`gateEligible=true`，issues/counter issues/console/page/WebGPU diagnostics 全为 0。
- 新 Gate 的 B/C active materials 为 `1/3`、fullscreen draw 恒为 `1`；B texture evidence 为 `resident/retiring/free = 4/0/59`，C 为 `0/0/63`，texture/sampler fallback 均为 0。Single Resolve P50/P95/P99 分别为 B `9.9478445/10.14003285/10.1460405 ms`、C `4.875299/5.04401315/5.08029693 ms`。该 adapter 与 2026-08-28 NVIDIA Turing 不同，这组只关闭新增语义的 clean correctness Gate，不做跨 GPU 性能百分比比较。
