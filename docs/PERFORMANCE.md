...
Hardware Raster 时间为 P50/P95/P99，单位 ms；RasterWork/alpha 为 sampled counter P50：

| Case | Hardware Raster | RasterWork | alpha RasterWork |
|---|---:|---:|---:|
| A | 约 `37.27 / 39.81 / 40.25` | `273,750` | `0` |
| B | 约 `39.92 / 40.18 / 40.33` | `≈281,191` | `0` |
| C | 约 `0.053 / 0.061 / 0.061` | `127` | `40` |

这是正确性 Gate，不是性能优化完成证明。相对 clean R3 `aff3ab8` 的 A/B Hardware Raster P50 `10.486/10.355 ms`，R4-A 在 RasterWork 数量基本不变时回退到约 `37–40 ms`。R4-A-03 新增的 per-fragment Material Visibility lookup/alpha 分支是待验证嫌疑，不是已证明根因；下一轮必须分别 profile record lookup、alpha branch/atlas 和额外 `r32uint` attachment 带宽，保持场景、分辨率、DPR、画质、warm-up 与采样 cadence 不变。R4-B 报告必须继续独立列出 `hardware-raster`，不得用总 Resolve 改善掩盖该回退。
