...
- Temporal/history differences: specular temporal保留surface与reflected-hit两个独立history candidate，各自执行4-tap geometry-weighted采样后再按hit trust/confidence合并；禁止先混receiver/hit velocity再采一个不对应物理点的中间坐标。surface history coordinate是receiver center减receiver velocity并以receiver depth/normal校验；hit history coordinate是当前full-resolution hit texel center先映射到effect domain、再减hit velocity，并以hit depth/normal校验，绝不能以receiver center/plane作为hit path原点。Three的hit candidate由world hit point与previous camera matrices投影；OEngine用同一GPU surface ABI中receiver/hit texel的 `current-previous` pixel velocity，hit texel若motion-invalid或reactive则拒绝该candidate，motion factor仍只取receiver path。OEngine用shared `OcclusionConfidence`（已经读取previous depth/current+previous camera）和`SurfaceValidity`代替额外previous-depth/normal history owner，并用当前motion-domain normal/depth约束两组4 taps。该适配把history texture fetch由4增至8但不增加attachment/pass，性能必须由Step 6 Gate决定。history valid、frame index、strength和pre-exposure都保留为FrameGraph execution-time binding，不能在cached recipe build时冻结。history alpha保留OEngine综合reflection confidence，而不是Three的inverse frame count；因此temporal aggressivity由有效history/confidence有界驱动，不能宣称逐式复制Three的frame-age公式。history pre-exposure记录multiplier/generation，兼容时按current/previous ratio重标定，不兼容时拒绝，且只有main command成功submit后才推进记录；camera cut、resize、scale、view、lighting、feature/format change与aborted submit继续由`TemporalHistoryRegistry`失效。`temporalEnabled=false`时两张history texture不分配。
