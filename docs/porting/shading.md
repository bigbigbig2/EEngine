...
- Temporal/history differences: specular temporal保留surface与reflected-hit两个独立history candidate，各自执行4-tap geometry-weighted采样后再按hit trust/confidence合并；surface coordinate/geometry来自receiver，hit coordinate/geometry来自当前hit texel。Three的hit candidate由world hit point与previous camera matrices投影；OEngine用GPU surface ABI中receiver/hit texel的`current-previous` pixel velocity，hit texel若motion-invalid或reactive则拒绝该candidate，motion factor仍只取receiver path。OEngine用shared `OcclusionConfidence`和`SurfaceValidity`代替额外previous-depth/normal history owner，并用当前motion-domain geometry约束两组4 taps；该适配把history fetch由4增至8但不增加attachment/pass。YCoCg variance clip恢复pinned HDR路径：current neighborhood按`1/(1+10L)`压缩、motion gamma为`0.5..1`、以box center clip、按original history scale恢复，并以motion clamp intensity和clip distance衰减confidence。reprojection Jacobian最小奇异值stretch guard已恢复；`dpdx/dpdy`在所有non-uniform early return之前执行，平方衰减history并参与clamp intensity，符合WGSL derivative uniformity。当前仍未实现Three的ray-length neighborhood edge/curvature hit trust；不能在文档中冒充已移植。history alpha保留OEngine综合reflection confidence而非Three inverse frame count，因此temporal aggressivity由有效history/confidence有界驱动。history/pre-exposure/generation均execution-time late-bound，只有main command成功submit后推进；camera cut、resize、scale、view、lighting、feature/format change与abort由`TemporalHistoryRegistry`失效。`temporalEnabled=false`时两张history texture不分配。
