...
- Recurrent differences/invariants: 保留固定 8-tap Vogel、view-space lobe projection、Three viewport mirror、`adapt=0.5` radius/polar feedback和 filtered-history/filtered-current 双分支。spatial edge weight的 luminance输入固定来自 raw current，并由该 weight驱动 kernel feedback；temporal history color/confidence不能改变 raw kernel形状。temporally filtered color另乘 history confidence，raw-current color只乘当帧 trace/raw validity，二者不能共用 history-weighted accumulator。这样 disocclusion时 current raw仍能被空间过滤，再进入 OEngine confidence-alpha Karis-style汇合；Three 的 inverse-frame-count alpha、前五帧逐 tap firefly weight和 environment-ray sentinel不直接采用，因为 OEngine miss以 confidence 0回退 baseline，不把 environment注入 SSR。
