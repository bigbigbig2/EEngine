...
- Temporal/history differences: OEngine 用 shared `OcclusionConfidence`（已经读取 previous depth/current+previous camera）和 `SurfaceValidity` 代替额外 previous-depth owner，并用当前 motion-domain normal/depth 约束 4 taps。history valid、frame index、strength 和 pre-exposure 都保留为 FrameGraph execution-time binding，不能在 cached recipe build 时冻结。history alpha保留 OEngine综合 reflection confidence，而不是 Three 的 inverse frame count；因此 temporal aggressivity由有效 history/confidence有界驱动，不能宣称逐式复制 Three 的 frame-age公式。history pre-exposure 记录 multiplier/generation，兼容时按 current/previous ratio 重标定，不兼容时拒绝，且只有 main command 成功 submit 后才推进记录；camera cut、resize、scale、view、lighting、feature/format change 与 aborted submit 继续由 `TemporalHistoryRegistry` 失效。`temporalEnabled=false` 时两张 history texture 不分配。
