...
- Provider selection: OEngine-authored `LongRangeDiffuseProviderPass` 在同一 fragment producer 内按 receiver 早返回执行 `Brick4 > Probe Volume > IBL > black`；不会先跑多个 fullscreen candidate。输出 alpha 编码唯一 provider identity，SSGI 只消费该 authoritative long-range product，不拥有 provider counters。Brick4/LPV owner generation 与 coarse residency 已接入；Brick4 mapping generation/required-brick residency 仍未冻结到 receiver ABI，所以 Step 5 仍不能关闭。
...
- Fallback/lifecycle: Brick4 owner exposes monotonic generation/non-empty residency/invalidate；LPV zero-version source is initialized on its first frame and becomes available only with generation parity plus non-empty tetra coverage。Probe work only runs after Brick4 spatial miss，IBL only after Probe miss，black is deterministic final fallback。`mode!=ssgi` 不创建 SSGI-only component MRT。Brick4 receiver mapping generation/required-brick residency and legacy mode deletion remain open。
- Local validation: schema v21 freezes provider、invalid-generation、nonresident、unassigned and duplicate counters；static source audit marks the provider shader authored-live/reachable。Type/build、GPU provider fixtures、feature matrix and PERF were deliberately not run in this round。
