...
- OEngine/WebGPU differences: 不移植 TSL、NodeMaterial、QuadMesh、three.js RenderTarget 或独立 `TRAANode`。输入是 OEngine full-resolution、working-linear、pre-exposed `PreExposedOpaqueRadianceSource`，并读取 shared reverse-Z Depth/HZB、compact world normal、Velocity、surface validity 和 shared occlusion confidence。公开 `samplingDomain = "world" | "screen"`：`world` 使用 `radiusMeters / metersPerWorldUnit` 经投影得到 pixel step，作为 OEngine 默认并保持物理尺度；`screen` 使用上游 `radius × (traceWidth / 2) / 16`，其中 `screenSpaceRadius` 默认 12、范围 `[1,25]`。两者只改变同一 trace 的采样半径，不创建新 owner/pass/resource；切换会使 SSGI history generation 失效。当前半分辨率 trace 写 `rgba16float AO/second-moment/oct-bent + rgba16float incident-GI/confidence`，joint spatial/temporal filter 后一次 full-resolution bilateral resolve 拆成 `r8unorm + rg16uint + rgba16float + r8unorm`。
