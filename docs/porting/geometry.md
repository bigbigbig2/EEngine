...
- Web differences: Range/glTF accessor canonicalization 在 Worker TS 边界形成 bounded canonical binary，保留 interleaved stride、normalized integer、index、material alpha/double-sided 和 canonical defaults；WASM 返回 Product tables、content-manifest identity evidence 和 page handle，只有取得 whole-page output credit 才逐页拷到 exclusive `ArrayBuffer`，并在 adapter 侧校验 decoded page hash。`WebCookWorkerHost` 只在 Dedicated Worker 侧持有 CPU/WASM cooker、转发 descriptor/page Transferable，并在 session generation/cancel/failure 时释放 source/handle；不先写 OEGPACK，不把 WASM memory/SAB 当 transferable，不创建 GPU object。当前只有 `portable-single` producer ABI、Node fake-module adapter/host tests；真实 Emscripten module bootstrap 和 Runtime admission 接线仍未交付。
