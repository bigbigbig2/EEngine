...
- Fallback/lifecycle: input/recipe/reserved/padding/index/finite/budget 错误整体 fail closed；opaque handle 在 generation cancel/failure 时整体销毁，未 offer descriptor/page 不允许进入 admission。`portable-pool`/pthread 是 S3 execution profile：pthread 变体已构建到 `wasm/vendor/threads/` 但 pool 在应用 Dedicated Worker 内的握手尚未闭环，默认仍是 `portable-single`。
- Local validation: Native ABI oracle 对同一 canonical cube 连续 cook 两次并逐 section/page byte compare；C++/TS canonical + recipe bytes 共用 SHA-256 golden；覆盖 length、normal declaration、non-finite、index 和 budget negative case。Emscripten build、Dedicated Worker 与 Range coalescing 已交付；`run:glb-web-product` 用真实 Dungeon GLB（798 mesh / 25 material）在 Chrome 跑通并做 HDR 像素覆盖率断言（region 256、litPixels 5610/65536）。仍缺 Nyx differential corpus、Worker crash/OOM、portable-pool/pthread 与 THIRD_PARTY 之外的性能证据。
...
- Validation: CPU metadata heap/record/generation/section negative oracle、非零 ProductTableSlot admission/rollback、V3 native golden；真实浏览器编译/readback 与 Product 生产像素证据已由 `run:glb-web-product`（真实 Dungeon GLB，HDR 覆盖率断言）提供；仍缺 Nyx DAG 与 VBuffer differential corpus、相同 workload GPU 性能证据，以及 `ProcessMeshletBatch`/`computeMain`/`VBufferMesh::BuildVertexOutput/meshMain/pixelMain` 的更细逐函数对照。
