...
- Local validation: `asset-codec-service.test.mjs` 覆盖 protocol、priority/FIFO、Worker/memory bound、Transferable、cancel/failure/replacement、lazy lifecycle、policy，并用固定 SHA-256 fixture 调用 pinned WASM 转 BC7；`runtime-asset-v2.test.mjs` 覆盖 package/residency。GPU consumer 的当前 gate 由 ADR-0014 validation registry/STATUS 管理。
