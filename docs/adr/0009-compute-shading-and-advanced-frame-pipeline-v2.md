...
`internalScale != 1` 时必须启用唯一 Temporal Reconstruction owner，由它把 `internal-full` HDR 显式重建为 `output-full`。`fixed` 只表示 scale 在采样窗口内不变，不提供绕过重建的隐式 bilinear upscale；Temporal Reconstruction 关闭时唯一合法值是 `internalScale=1`。因此 Final Output、FinalColorPyramid、Motion Blur 与其他 output-domain consumer 永远不会直接把 internal-domain ResourceId 当作 output-domain texture 使用。
...
移植 Three.js `GTAONode` 的 horizon/slice 数学、sampling distribution、radius/thickness/falloff、depth/normal reconstruction，并用 OEngine FrameGraph、主 Depth mip0、resolution domain 和 temporal/history owner 实现；Shared HZB只属于经验证需要层次 traversal 的consumer，不是GTAO精确horizon采样输入。扩展同一 GTAO producer 输出 `ScreenAmbientVisibility + BentNormal`，保留 joint depth/normal resolve；不得为了 bent normal 继续运行旧 GTAO，也不得引入第二套 Three.js TRAA/RenderTarget owner。
...
**Implementation record（2026-09-12，production cutover + corrected MILESTONE verified，PERF/运动视觉复核 open）**
...
- 进一步的原分辨率视觉复核发现，raw horizon trace 虽然已使用合法 depth binding，却同时把 shared HZB 的保守 2×2 footprint 当成精确遮挡深度，导致薄 bar/cube 被放大成跨屏宽暗带。当前 production GTAO 已删除 HZB binding 与所有 raw trace HZB lookup，固定读取主 `depth32float` mip 0 精确 texel；Shared HZB 仍由唯一 owner 生成并服务真正需要层次 traversal 的 SSGI/SSR。该改变没有增加 pass、texture、submit 或 readback，反而减少 GTAO 的一个 sampled binding。
- GTAO Pass ABI进一步把Velocity、shared `OcclusionConfidence`与`SurfaceValidity`冻结为temporal-only输入。`screenSpaceDiffuseTemporal=false`时只建立linear-depth、raw trace、spatial与joint resolve，不导入history、不读取motion/disocclusion/classification；旧主管线用`depthRes`伪装velocity/confidence的dummy binding已经删除。temporal打开时history bindings与三种时域资源必须同时存在，否则构图显式失败；这使non-temporal GTAO可与previous-depth/Velocity/shared disocclusion的feature-off裁剪真正闭合。GPU sampled `aoHistoryAcceptedPixels/RejectedPixels` 也已绑定同一`SurfaceValidity`，使用与production temporal一致的effect-pixel中心映射、3×3最大Velocity、128 px velocity confidence、disocclusion confidence、`motionValid && !reactive`、reprojection bounds和`>0.001`分支。旧counter路径漏掉classification且full-resolution映射缺少`+0.5`中心偏移，会把reactive/motion-invalid或跨采样边界像素误报为history accepted，现已删除该近似。
- 历史 `advanced-frame-pipeline-v2-step4.json` 记录修正前 candidate 的三个独立 Chrome context：518,400 GTAO pixels、GPU P50/P95 1.962/2.284 ms、相对 Step 3 phase P50 +0.089 ms。由于本次把 step rand 从本地 hash 收敛为 pinned Three.js 公式，旧 artifact 只作为历史对照，不能证明当前实现性能。
- exact-depth修正之前的 canonical `surface.gtao-replacement` 已由 Playwright 启动本机 Google Chrome 153 在 NVIDIA Turing adapter 上通过全部 11 项断言：1280×720 internal、640×360 half-resolution、`3 directions × 6 steps × 2 sides = 36` depth samples/pixel，raw/spatial/temporal/joint resolve 各一次，one-main-submit、feature-off 无 pass/history/transient/counter 残留，WebGPU validation/uncaptured/device-lost 均为零。它只能作为 topology/counter 历史证据，不能证明当前 shader revision。
- 当前 exact-depth revision 已在同一本机真实 Chrome 以 1600×900 contact fixture、40 stable frames、`off/on/raw/spatial/temporal/normals/depth` debug矩阵逐张按原分辨率人工复核：宽暗带消失，浮动 bar/cube 下方只保留局部接触遮蔽，真实几何间隙可辨，temporal结果稳定。随后 corrected canonical `surface.gtao-replacement` 重新通过全部11项断言：230,400个half-resolution GTAO pixel、36 depth samples/pixel、四阶段各一次、feature-off零资源、one-main-submit，WebGPU validation/uncaptured/device-lost均为0。运动镜头矩阵和本ADR唯一clean `comprehensive-full` formal run仍open，不能由静态fixture替代。
...
**Implementation record（2026-09-12，production cutover + MILESTONE verified，PERF/扩展视觉复核 open）**
...
- SSGI temporal输入与spatial/trace输入已经在Pass ABI中分离：Velocity、shared `OcclusionConfidence`、`SurfaceValidity`和四张history只在`screenSpaceDiffuseTemporal=true`时必需、绑定和读取。旧non-temporal路径为了复用history evidence pipeline，把`albedoAo`伪装成三种时域纹理；虽然`historyValid=false`阻止其成为颜色结果，但仍制造无意义绑定/读取，并把未启用history的像素错误累计到`ssgiHistoryRejectedPixels`。现以两个静态evidence variant取代：trace-only variant只绑定counter/settings并统计实际`ssgiEvaluatedPixels/ssgiTraceSamples`；temporal variant才绑定Velocity、disocclusion与`SurfaceValidity`并统计accepted/rejected，且逐像素镜像production temporal的 `history valid + PreExposure scale + reprojected UV bounds + motionValid + non-reactive + disocclusion confidence + blend` gate。旧temporal evidence漏掉`SurfaceValidity`/UV/PreExposure/blend，却凭空使用production shader不存在的`length(velocity) < 128`阈值，因此counter不能证明真实history decision；该近似已删除。non-temporal accepted/rejected保持零，不创建伪时域依赖。evidence dispatch仍只在GPU counter采样帧存在，不进入普通未采样帧成本。
...
- `surface.ssgi-production` 定义并已执行两阶段 Brick4 oracle：resident generation 的覆盖 receiver 只计 Brick4；推进 expected generation 后 Brick4 为零、`invalid_generation > 0` 且 receiver 回退 IBL。同时以 `samplingDomain=screen, screenSpaceRadius=12` 覆盖 pinned native-radius GPU branch，并保留 pinned path、GTAO owner/history 缺席、source-before-resolve、component product、temporal closure 与 one-main-submit 检查；静态 oracle 冻结 temporal-off direction/offset=1、Three.js rand/IGN、双 radius branch、viewport break 与 history invalidation。
- 新版 Chrome 验证首次暴露三项真实生产缺口并已修复：WebGPU 对 storage buffer 静态布局要求 Brick4 unavailable dummy 至少 48 B，现由 CPU/WGSL owner共享 `BRICK4_LIGHT_MAP_MIN_BINDING_BYTES` 并在 BGL 固定 `minBindingSize`；GPU counter active field 是逐帧注册，SSGI counter 现随最终 `graphTopology.ssgi` 每帧登记，而不是只在 service 创建帧登记；reverse-Z clear plane 的 `depth <= 0` receiver 现于 provider 选择与counter之前早退，背景不再污染 IBL ownership。
- 修正后 `surface.ssgi-production` 在同一本机 Chrome/adapter 上通过全部 11 项断言，GPU sampled evidence 为 `evaluated=230400`、`accepted=26137`、`rejected=204263`；有效 Surface receiver 为 104,570，provider receiver总数与其相等，unassigned/duplicate 为零，Brick4 stale-generation 回退 IBL成立，WebGPU diagnostics 为零。canvas fixture 已人工检查为可见场景；world-radius与长时间temporal/bent-normal画质仍需扩展视觉复核，性能只由后续 clean formal run 判定。
...
- 对 near-delta material 增加了明确的 estimator specialization：perceptual roughness `<= 0.04` 时发射确定性 mirror ray，超过阈值仍走 Three bounded GGX VNDF。原因不是引入第二条 SSR 管线，而是 one-spp stochastic estimator 在近镜面极窄 lobe 上只制造可见salt-and-pepper、没有可感知roughness收益；该分支复用同一 trace/hit-shading shader、HZB、packed ABI、filter chain和submit。
...
- 原实现把 Three `RecurrentDenoiseNode` alpha 的 inverse accumulation age 直接等同于 OEngine `rgba16float.a` 的 reflection replacement confidence，这是语义错误：高置信 hit 反而会被解释成“刚出现的 sample”，造成kernel收缩和长期闪烁。当前 Temporal history rate 与输出confidence分离：有效current使用`mix(0.85, 0.97, currentConfidence)`；current stochastic miss不再立即清零，而只在3×3 receiver邻域存在screen-hit evidence时以`0.92`衰减复用receiver history；真正无screen evidence时confidence仍为0并保留Local Probe/IBL baseline。Three的inverse-age行为只有未来增加独立age ABI后才能恢复，禁止再次借用replacement confidence冒充。
- RecurrentDenoise 保留固定8-tap golden-angle Vogel、view-space specular-lobe tangent basis、Three viewport mirror、5-tap inverse-length ray statistic、REBLUR frustum-normalized hit-distance、GGX inverse-CDF normal lobe和`adapt=0.5` feedback结构。OEngine没有独立age通道，因此recurrent history aggressivity改为独立、有界的`0.9 × temporalStrength`，不再从output confidence派生；默认strength 0.9时aggressivity为0.81、Karis raw-current权重为0.19，既允许持续响应又不会像旧0.15 aggressivity那样让85%以上raw噪声重新覆盖Temporal结果。radiometric/ray edge系数固定`0.02`，roughness boundary固定`0.15`，depth/normal继续作为权威几何edge stop。projected world footprint被OEngine scale-agnostic场景放大时，8个tap不全部坍缩到同一边界点，而以`radiusFraction=i/7`、`maximumRadius=1+7×radiusFraction²`形成`1..8` half-resolution pixel的quadratic multi-scale footprint：前tap填补局部stochastic hole，后tap保留粗糙反射的宽核。center或sample invalid时raw accumulator从零权重开始，禁止phantom black center；miss可以由有效邻居填补，但不得更新radius/polar feedback。temporally filtered分支额外乘sample history confidence，raw-current分支独立乘当帧trace/raw validity，两者最后才做Karis-style汇合并写双缓冲external history。half-resolution joint bilateral upscale也改为对RGB做confidence-weighted geometry filter、对confidence取geometry-weighted maximum，避免零置信miss作为黑色样本被平均两次。所有改动不增加tap、pass、texture、submit或readback；更宽固定8-tap footprint与5-tap ray statistic必须由Step 6 PERF验收。
- `temporalEnabled=false` 时不再构造两张 SSR history；recurrent pass直接从 trace word恢复 current confidence。该 spatial-only SSR topology 也不再把 Velocity、shared `OcclusionConfidence`、opaque `SurfaceValidity` classification或previous-depth误列为必需输入：三项 `ScreenSpaceReflectionsInputs` motion/disocclusion字段在ABI上是optional，只有`ssrTemporal=true`时才强制同时提供它们和history bindings并建立TemporalReproject reads。主管线的SSR构图guard同样只在temporal specialization内检查这些资源，non-temporal SSR只支付trace、hit shading、recurrent/conditional upscale和replacement所需成本。SSR feature关闭时 `ReflectionService` owner、trace/prefilter/temporal/denoise/upscale、history、baseline-specular consumer 与 correction 仍由 topology整体裁剪。
...
- 间接高光遮蔽统一改为可追溯的 Filament spherical-cap/cone intersection helper：`OpaqueLightingResolve` 与 `ScreenSpaceDiffuseResolve` 共享 reflection direction、bent normal、roughness、visibility 四输入的同一公式，删除旧的单变量`smoothstep`近似。与此同时 `SURFACE_PBR_DEBUG_WGSL` 的PBR输入修正为与production `rg32uint` attachment一致的`texture_2d<u32>`；此前错误的float声明会使roughness debug在真实WebGPU下bind失败，视觉矩阵无法证明材质边界。
...
- 稳定性修正之前的 canonical `surface.ssr-replacement` 曾在本机 Chrome/NVIDIA Turing 上通过全部 11 项断言：真实 HZB trace统计 `tracePixels=127029`、`hitPixels=26252`、`traceSteps=2401880`、max step 128，trace/hit-shading/temporal/recurrent/upscale/correction闭环、one-main-submit、temporal-off history裁剪和SSR feature-off零成本成立，WebGPU diagnostics为零。该结果仍是 topology/counter 历史证据，但不能证明当前shader revision。
- 当前 revision 已以真实 Chrome、1600×900 road fixture、50 stable frames 对 `off/on/hit/resolve/temporal/confidence/roughness/normals` 做原分辨率人工复核。修正前 near-mirror 与 roughness strip 出现高密度salt-and-pepper和大面积离散点切换；分离alpha语义、允许有邻域hit evidence的miss history复用、扩大固定8-tap的多尺度footprint、把稳定history aggressivity修正为0.81并采用confidence-aware upscale后，near-mirror strip连续，roughness 0.38 strip只保留低幅细颗粒/渐隐。raw hit debug按定义仍呈现stochastic samples，confidence debug仍有细粒度变化，不能据此伪称raw estimator无噪声。随后 corrected canonical `surface.ssr-replacement` 重新通过全部11项断言：127,029 trace pixels、26,252 hits、2,401,880 steps、max 128，temporal/history closure、temporal-off和feature-off裁剪、one-main-submit均成立，WebGPU validation/uncaptured/device-lost为0。camera motion/exposure矩阵和clean `comprehensive-full` PERF仍open。
...
**Implementation record（2026-09-12，production ownership + MILESTONE verified，PERF open）**

- 新增 typed `OpaqueColorPyramidFrame` 与 `FinalColorPyramidFrame`。前者强制 `internal-full + post-screen-space-diffuse-pre-ssr`，后者强制 `output-full + post-transparency-temporal`；两者分别携带 source generation、mip count、PreExposure 和 domain validation。`FinalColorPyramidFrame.source` 明确指向未 Bloom composite、进入 shared producer 前的精确 output-full HDR ResourceId，`texture` 才指向具有 mip chain 的独立 pyramid；factory 现拒绝两者使用同一 ResourceId。旧接线错误地把两者都发布为 pyramid texture，导致 Bloom capture composite/无 composite identity 名义上回到 source、实际却读取复制后的 mip0。修正不删除共享 pyramid copy（其 mip consumer 仍需要），只恢复 typed product 的 source/texture 双资源语义。为了保证该精确 source 在所有拓扑下都是真的 `output-full`，设置校验同时禁止 Temporal Reconstruction 关闭时使用任何 sub-native `internalScale`；native temporal-off 与 temporal-on reconstruction 两条合法路径都在进入 shared producer前得到 output-domain source，不再依赖 pyramid mip0 copy 暗中承担未声明的 upscale。
...
- Shared Depth/HZB 已经由 per-view `HierarchicalZBuffer` 以一张 current `rg16float` mip hierarchy、一次 compute pass和多 mip dispatch生产；SSGI/SSR读取该同一 FrameGraph resource，GTAO则因horizon integration需要精确texel而只读主depth mip0。本 Step 没有为了形式统一重写一个已经闭合的 producer，也没有让任何 effect增加私有 HZB。canonical case明确检查至多一个 `hzb_current`、一个 graph producer、一个 build和有实际层次consumer时的正数 mip dispatch。
...
- `FrameContext.history` 现包含 color/GTAO/SSGI/SSR/NSS-feedback/Exposure validity；公开 `sharedDerivedProductsEvidence()` 与 `sharedPyramid.*` profiler counters报告两种 build/render-pass/mip/bytes、consumer count、Bloom reconstruction、Exposure metering extent以及全部 history声明。新增 `surface.shared-derived-products` canonical real-Chrome case，覆盖单 producer共享、source-stage顺序、SSGI source不 alias、SSR-only/Final-only/all-off裁剪、camera cut、output resize、GTAO↔SSGI reset、one-main-submit和 profiler一致性；所有资源存在/缺席断言统一只读取 compiled dump 中 `firstUsePass !== undefined` 的 live resource，零 use 的 culled declaration 不再冒充生产资源。source-domain selector会由 shared/post/temporal/SSR/SSGI 代码变更选中该 case。pure ABI tests另外冻结 Final product validation、提交后推进、abort、feature reset、exposure ratio与 non-HDR history隔离。
...
- `surface.shared-derived-products` 已在本机 Chrome/NVIDIA Turing 上通过全部 14 项断言：opaque/final pyramid各自只由一个shared owner生成，SSR/Bloom/Exposure读取匹配source stage，SSGI full-resolution source不与二者伪alias；SSR-only、Final-only、all-off、camera cut、resize、GTAO↔SSGI reset、one-main-submit和live-resource profiler口径均闭合，WebGPU diagnostics为零。canvas fixture已人工检查为正常可见。该 MILESTONE 证明owner、lifecycle和feature pruning，不把portable multi-pass producer宣称成SPD/subgroup性能等价物；最终性能仍等待clean formal run。
...
**Implementation record（2026-09-12，production reconstruction + MILESTONE verified，PERF/扩展视觉复核 open）**
...
- shared `OcclusionConfidencePass` 同样直接读取当前与上一帧两张主管线 `depth32float`，第三个 sampled binding 才是 velocity。两个深度 binding 现分别固定为 depth-only view、WGSL `texture_depth_2d` 与 BGL `sampleType: depth`；4-tap previous-depth gather 和 3×3 current closest-depth helper也都接受 depth texture并直接使用标量 load。旧 `texture_2d<f32> + unfilterable-float + .r` 路径无法与实际 depth attachment 建立合法 bind group，且含混的 shader 变量名会掩盖三个 binding 的真实语义；现已改为 `current_depth_source / previous_depth_source / velocity_source`，不改变既有 reverse-Z conservative rejection 数学。其 `r8unorm` 输出只被所有 consumer 以 mip0 `textureLoad` 读取，因此资源固定为单 mip；旧完整 mip chain 从未被生成或消费，只产生约 1/3 的理论额外纹理 allocation，删除它不改变 pass、submit 或采样语义，实际 bytes/性能收益仍等待 FrameGraph/浏览器证据。
- 主 `depth32float` 只被所有 attachment/sample/copy/HZB source view以 mip0 使用，现从固定5 mip收缩为单 mip；hierarchy仍由独立 `rg16float` HZB完整生产。previous-depth本身也从无条件双缓冲改为consumer-driven owner：仅当`screenSpaceDiffuseTemporal || ssrTemporal || temporal`至少一个成立时，才导入`previous_depth` FrameGraph resource、创建shared `OcclusionConfidencePass`并持有第二张`depth32float`。普通non-temporal SSR不属于该集合，因此不会仅因SSR开启而生成Velocity、opaque temporal classification、disocclusion confidence或第二张depth。全部previous-depth consumer关闭时，当前奇偶slot被保留为下一帧唯一depth，另一slot在submitted-work边界后退役；再次开启时，最后一次已提交的单depth被放入previous parity，另建current slot。帧号只在成功提交后推进，所以encode/submit abort重试不会交换错误的current/previous角色。`TemporalRuntimeEvidence`与Profiler分别报告`previousDepthConsumed`、`mainDepthTextureCount=1|2`和`previousDepthBytes`，总memory evidence也把previous depth列为独立history owner。按几何级数，单mip相对旧每张5-mip allocation理论减少约25%；feature-off再消除整张previous depth，但实际bytes、动态切换正确性与GPU收益仍等待FrameGraph/浏览器证据。
...
- Reprojection 仍使用 Surface ABI 的 `current - previous` internal-pixel velocity，并从 reverse-Z 3×3 邻域选择 closest foreground depth对应的 velocity/motion validity。final-layer transparent reactive 继续取 output pixel并与 selected opaque surface reactive 取最大值，避免 MBOIT foreground因为 closest-depth选择后方 opaque 而丢 mask。TAA shader 现在在任何 history sample 前严格拒绝 global/generation/pre-exposure invalid、motion-invalid、`reactive >= threshold`、disocclusion confidence不足和 reprojected UV越界；删除了“reactive foreground允许 motion-invalid history”的隐式例外。NSS preprocess也已成为同一Reactive/SurfaceValidity合同的真实consumer：selected closest-depth pixel提供Velocity、motion validity与shared occlusion confidence，current/selected reactive取最大值，UV越界或PreExposure无效时把network输入history validity归零；partial reactive连续衰减有效性。旧NSS把名为`disocclusionConfidence`的输入按`1-confidence`使用，导致稳定像素拒绝history、disoccluded像素接受history，且完全未读取SurfaceValidity。现直接使用confidence正向语义，并把3×3 offset index与最终per-pixel validity打包进既有`rg8unorm`的R/G，resolve再以同一validity硬门控history weight；没有新增texture、pass、submit或扩大2 B/px中间ABI。
- SurfaceValidity producer 已按透明内容边界去重。旧图只要主TAA/NSS开启，就在供GTAO/SSGI/SSR Temporal使用的opaque classification之外无条件再生成一张同尺寸、同metadata的final classification；无透明几何时二者内容完全相同，却多一次full-screen render与一张internal `rg8unorm`。现在`graphTopology.transparency=false`时，唯一opaque classification同时作为final Temporal输入，主owner/history/PreExposure counter job也挂到该producer；`graphTopology.transparency=true`时才在OIT reactive之后生成第二张final classification，并要求transparent reactive真实存在，禁止以disocclusion或其他dummy替代。公开`classificationPasses`按`Number(screenSpaceDiffuseTemporal || ssrTemporal || temporal) + Number(temporal && transparency)`报告实际0/1/2 pass；该优化不改变closest-surface、reactive或history数学，但实际GPU收益仍等待Step 8 PERF。
...
- CPU `TemporalResolveContract.classifyTemporalHistory()` 与 TAA WGSL 的 hard reject、relative luminance、reactive smoothstep和 confidence-gated lock推进保持同一公式。GPU sampled evidence不再把 reactive/disocclusion阈值硬编码为常量，也不再以同坐标 classification/confidence 冒充 production consumer：32 B uniform late-bound history/PreExposure validity、`taa|nss` owner、internal/output extent和同一 `RenderSettings.temporal` 诊断阈值；counter-only pass才额外读取主管线 depth/velocity。TAA specialization在 output extent复演output→internal映射、TAA遍历顺序的reverse-Z 3×3 closest surface、selected/current reactive、selected confidence/motion和严格 `<1` history bounds，因此`temporalHistoryRejectedPixels`对应实际hard reject。NSS specialization在internal extent复演其不同的closest遍历顺序、selected/current reactive、正向continuous confidence、motion/global/PreExposure validity和`<=1` bounds，并按写入`rg8unorm`后会量化为零的`0.5/255`边界统计hard rejection；它不错误套用TAA阈值。`temporalReactivePixels`与`temporalDisoccludedPixels`继续按当前配置阈值作为跨owner诊断分类，不声称是NSS hard gate。
- `RenderSettings.resolution` 从单个 scale 扩展为唯一 fixed/adaptive policy：`mode`、current/fixed `internalScale`、adaptive min/max、target frame rate、tolerance与 settle frames。默认及 Renderer `renderScale` 均为 `fixed, scale=1`；手工设置 `internal_resolution_scale` 会显式切回 fixed，formal benchmark profile也显式写 `mode=fixed`。任何 fixed/adaptive sub-native `internalScale` 在 Temporal Reconstruction 关闭时都配置失败；adaptive 即使当前 bucket 为 1 也要求 reconstruction owner，因为后续可降档。该统一约束防止 Final Output、Motion Blur 或 FinalColorPyramid 把 internal-domain source误作 output-full，也防止后台 DRS 绕过重建。
...
- 新增 pure ABI/oracle覆盖 typed reconstruction domain/confidence、fixed DRS完全不消费 timing、adaptive warm-up后只落到合法 bucket、fixed native settings默认、fixed sub-native无Temporal拒绝、invalid adaptive range、CPU/WGSL reactive/motion/pre-exposure/9-tap invariant、无透明classification复用/透明reactive强制输入，以及 Occlusion Confidence 当前/上一帧 depth view/BGL/WGSL 一致性。新增唯一 canonical real-Chrome `surface.temporal-reconstruction` case：以 fixed 0.75 internal scale检查 output-domain TAA closure、无透明时单个共享classification→resolve、唯一 history read/write、one-submit、fixed timing sample不增长；透明子场景则要求opaque+final恰好两个classification且final真实消费OIT reactive。随后检查 camera cut/resize当帧 read-invalid和提交后恢复，以及 adaptive 0.67..1 policy可观察，最终回到 fixed 0.75保存 screenshot/counter artifact。changed-path routing覆盖 DRS、TemporalFeature/Resolve/Classification、FrameProducts/MainPipeline/RenderSettings和相关 WGSL。
...
- 真浏览器执行还发现 runtime evidence 原先在 scene runtime解析前按预备 `featureTopology` 统计classifier，透明场景compiled graph实际已有opaque+final两个producer却报告1；现改为只按最终 `graphTopology` 计算 `0|1|2`，并由源码oracle冻结。修正后的 `surface.temporal-reconstruction` 在本机Chrome/NVIDIA Turing上通过全部11项断言：fixed 0.75 internal→output TAAU、无透明单classifier复用、透明opaque+final双classifier与真实OIT reactive输入、唯一history read/write、camera-cut/resize read-invalid→提交后恢复、adaptive policy可观察、one-main-submit和WebGPU diagnostics零错误均成立。最终0.75截图已人工检查为正常可见；static subpixel、运动边缘、MASK/foliage、SSR/transparency长序列仍属于扩展temporal visual review，clean fixed formal负责性能与可重复性。
...
**Implementation record（2026-09-12，production fusion + MILESTONE verified，PERF/另一显示模式 open）**
...
- Render Debug仍覆盖最终scene HDR而不改写history。debug topology即使scene配置Bloom/grading/sharpen也选择三者全false的Final Output source variant，仅保留既有Exposure/display mapping；debug source显式固定在post-temporal/motion-blur、pre-post-effects HDR，与post capture同帧开启也不改变观察值。没有debug consumer的Bloom reconstruction会被FrameGraph裁剪；shared-product evidence的`opaqueConsumerCount/finalConsumerCount`现从最后compiled graph中non-culled的`SSR stochastic hit shading`、`Bloom reconstruct from FinalColorPyramid`与`Automatic exposure histogram eC`计算，避免SSR/Bloom配置开启但被debug recipe裁剪时仍虚报consumer。Automatic Exposure继续从pre-Bloom `FinalColorPyramid` lowest mip生成adapted scalar，不被融合为full-resolution scan，也不改变提交感知history slot。
...
- 新增公开bounded `FinalOutputRuntimeEvidence`，不泄漏GPU handle：报告最后compiled graph的实际`outputMode=sdr|hdr|none`与`GPUTextureFormat`、Final Output pass数、Bloom/ColorGrading/Sharpen各自是否融合、capture-only Bloom composite/ColorGrading materialization pass、standalone Sharpen pass、三个已知output-full HDR intermediate实际resource数、debug bypass和one-shot materialization。mode/format冻结在`MainFrameGraphRuntimeEvidence`生成时，不用可能已在帧间改变的`matchMedia`当前值改写上一帧；debug bypass同样从最后compiled graph的live `Render debug/*` pass判定。HDR intermediate只统计具有`firstUsePass`的资源，零use/culled declaration不冒充实际roundtrip。`FrameResourceSummary`保留既有declared `imported/transient`计数以兼容旧artifact，同时新增`liveImported/liveTransient/liveTransientTextures/liveTransientBuffers`；Inspector active摘要、LPV topology资源计数oracle与baseline-specular profiler均改读live口径。Profiler同步发布`post.finalOutputPasses`、三个fused flag与`post.fullResolutionHdrIntermediates`；GPU phase classifier将`Final Output`继续归入post而不是unknown。
- pure ABI/source oracle冻结无effect与full-effect binding plan、SDR plain variant物理缺少Bloom/grade/sharpen表达、HDR full variant包含Bloom/grade/neighbor load、Main graph normal fusion/capture exception以及不再调用standalone Sharpen。新增唯一canonical real-Chrome `surface.post-fusion`：all-on normal frame要求一个Final Output、实际display mode与`SDR + preferred canvas format`或`HDR + rgba16float`标签/格式配对、零 live `Bloom composited/Color graded/Sharpened` resource、零独立三pass和one-submit；Bloom/Sharpen off要求静态裁剪且不存在 live Bloom reconstruction resource；随后连同Automatic Exposure关闭时要求Final pyramid/Histogram/Exposure history全部裁剪、consumer/build均为零；one-shot capture要求恰好两个HDR materialization、finite rgba16float readback且下一帧恢复zero-intermediate fusion。该 case 现额外切换到 `RenderDebugView.LinearHdr`，要求Final Output的Bloom/grading/sharpen全部关闭、配置仍为on但无消费者的Bloom reconstruction pass/resource被裁剪、Automatic Exposure成为 `FinalColorPyramid` 唯一 live consumer，然后恢复普通fusion topology。Browser oracle的全部FrameGraph资源判断与runtime evidence统一采用`firstUsePass !== undefined`口径；static source oracle冻结两种shader label、mode/format记录、live资源过滤、Exposure-off与debug consumer断言。单次运行只验证当前显示器暴露的动态范围；另一模式必须在对应SDR/HDR宿主上另跑，不能以static shader存在冒充双模式运行证据。
- 本Step没有引入新的外部算法。Color Grading继续沿用`SHADE-GRADING`已登记的Filament invariant reference；Bloom reconstruction、RCAS-like bounded sharpen、SDR ACES近似、HDR GT7路径均保留现有OEngine实现，本次只改变资源/Pass ownership和表达组合。不能把“融合代码存在”当作性能提升；当前验收只要求固定adapter/browser/output/internal resolution/画质下的绝对HDR traffic、post GPU phase与整帧PERF，不要求新旧A/B或相对收益；另行声称性能改善时才需要比较证据。
- 初次新版Chrome运行先暴露counter capability ledger缺项，随后又暴露capture oracle在异步HDR readback期间被连续render loop下一帧覆盖graph evidence。前者已补齐所有live counter capability并增加schema/ledger key-set相等门禁；后者在发起readback前对capture帧graph/output evidence做不可变快照，避免把下一帧fusion拓扑误判为capture拓扑。
- 修正后的`surface.post-fusion`在本机Chrome/NVIDIA Turing上通过全部12项断言：当前宿主为SDR/bgra8unorm，一个Final Output，normal frame零live `Bloom composited/Color graded/Sharpened` full-resolution HDR中间资源；Bloom/Sharpen/Exposure关闭、LinearHDR debug bypass、Final pyramid最后consumer裁剪、one-shot post-color-grading capture两次必要物化与下一帧恢复fusion均闭合，one-main-submit且WebGPU diagnostics为零。fixture截图与capture数值已检查；HDR宿主模式仍必须在HDR显示链环境另跑，性能由clean formal Gate判定。
...
- old default AO在当前源树中没有并行SSAO/legacy owner；唯一`mode=gtao` owner是Three-derived `GtaoPass`。old SSR也没有并行reflection pipeline；唯一SSR service指向Three-derived trace/resolve/recurrent-denoise/temporal chain和baseline replacement correction。Step 4–6的canonical Chrome replacement oracles与综合topology matrix现已通过；仍不以单帧fixture替代扩展visual和final PERF。
- duplicate pyramid audit与真实Chrome `surface.shared-derived-products`均只找到一个`SharedColorPyramidPass`：SSR消费`OpaqueColorPyramid`，Bloom/Exposure消费`FinalColorPyramid`；SSGI继续直接读取语义不同的full-resolution pre-SSGI source。effect-local SSR scene-color pyramid、Bloom downsample pyramid或Exposure full-resolution reduction没有生产owner；不会为了减少类名而错误合并语义不同的opaque/final source。
...
- Step 9 normal production graph与Chrome oracle均证明不再调用`SharpenPass`，Final Output WGSL静态variant和SDR/capture路径已通过MILESTONE。严格按“replacement通过最终PERF后才删除”，`SharpenPass.ts`/`sharpen.ts`暂保留为final PERF后的明确deletion target，但它没有公开switch、normal FrameGraph consumer、GPU allocation或submit。capture-only `ColorGradingPass`不是dead code；它是`post-color-grading` instrumentation boundary的唯一物化owner，不得与Sharpen一起删除。
- source oracle增加counter schema v22的退役字段禁止回流与reserved-hole断言；schema与`BenchmarkCapabilityEvidence`现在还必须做完整key-set相等检查。Chrome首帧曾暴露SSGI四项与long-range provider八项虽然已有GPU producer/schema slot，却漏登记capability declaration，导致counter sample注册对`undefined.status`崩溃；现已补齐各自真实producer说明，`assertSupportedGpuCounter()`对未来遗漏也先抛明确RangeError而不是TypeError。既有Step 2–7 ABI/source oracle继续检查被删文件不存在、不含`PackedMaterialClassDepthPass`、Surface V1不回流、GI无scene-wide mode、single-valued screen-space diffuse topology以及共享pyramid唯一producer。
- 当前MILESTONE在同一工作树通过TypeScript、production build、WGSL source组合、ABI/source/documentation suites与examples验证工具；六个canonical Surface Chrome cases（GTAO、SSGI、SSR、shared products、Temporal、Post）的全部scenario断言通过，browser diagnostics全零。`rendering-lab.pipeline-matrix`在1920×1080、DPR1、`comprehensive-full`上执行base/full及九个`full-minus-*`，每路固定measured窗口均为main-submit P50=1、stable graph invalid frame=0、所有overflow maximum=0、browser errors=0。dirty-tree DEV profile只用于编排和风险发现，不作为正式性能证据。ADR-0009整体completion与Sharpen dead-source删除仍等待clean commit上的唯一`comprehensive-full` formal run及剩余扩展视觉/HDR宿主Gate；可变测试总数和当前性能采样只记入`STATUS.md`与可复算artifact，不在ADR累积。
...
- Temporal/DRS：代表性 camera sequence 覆盖 static、motion、disocclusion、transparency、SSGI、SSR、camera cut 和 resize；正确性与当前绝对 PERF 使用 fixed scale，adaptive 只做 bounded/hysteresis smoke。
...
PERF 重点报告当前 GPU frame envelope、classification/shading/lighting/effect phase、Surface/HDR/history bytes、shading utilization、material evaluation、GTAO/SSGI/SSR work 和 P50/P95。综合 workload 必须固定 adapter、分辨率/DPR、internal scale、warm-up、camera path、内容、feature topology 和 budget；不要求双基准体系。单个算法 candidate 先使用当前实现的 MILESTONE correctness/short profile，不要求旧算法 A/B；只有 production replacement、重大 keep/reject 或 ADR final 才进入当前 revision 的 formal group。
