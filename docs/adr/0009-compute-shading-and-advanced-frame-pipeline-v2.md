...
- TAA closest-depth search直接读取主管线`depth32float` attachment；其WGSL binding现冻结为`texture_depth_2d`，BindGroupLayout对应`sampleType: depth`。旧实现错误地把同一depth view声明成`texture_2d<f32> + unfilterable-float`，会在真实WebGPU bind-group validation阶段失败；修正后`textureLoad`仍返回同一reverse-Z `f32`，不改变3×3 closest-foreground选择或任何时域权重。
