...
- 修正后的 canonical `surface.gtao-replacement` 已由 Playwright 启动本机 Google Chrome 153 在 NVIDIA Turing adapter 上通过全部 11 项断言：1280×720 internal、640×360 half-resolution、`3 directions × 6 steps × 2 sides = 36` depth samples/pixel，raw/spatial/temporal/joint resolve 各一次，one-main-submit、feature-off 无 pass/history/transient/counter 残留，WebGPU validation/uncaptured/device-lost 均为零。保存的 canvas fixture 已人工检查为非黑、几何与遮蔽区域可辨；它证明接线和测试场景可读，不单独证明所有薄几何、长时间运动稳定性或 AAA 画质。当前 pinned-rand 实现仍需由本 ADR 唯一 clean `comprehensive-full` formal run 与扩展视觉矩阵关闭 PERF/quality Gate。
...
**Implementation record（2026-09-12，production replacement + MILESTONE verified，PERF/扩展视觉复核 open）**
...
- 新版 Chrome 首次编译真实 correction consumer 时发现 `specular_correction.ts` 错误组合了只含部分material定义的 `GPU_COMPUTE_MATERIAL_ABI_WGSL`，导致缺少 `OENGINE_SURFACE_DEFINED_FLAGS_MASK`；现改为组合权威 `GPU_SHADING_SURFACE_LITE_WGSL`，并由source oracle防止 partial snippet 回流。修正后的 `surface.ssr-replacement` 在本机 Chrome/NVIDIA Turing 上通过全部 11 项断言：真实 HZB trace统计 `tracePixels=127029`、`hitPixels=26252`、`traceSteps=2401880`、max step 128，trace/hit-shading/temporal/recurrent/upscale/correction闭环、one-main-submit、temporal-off history裁剪和SSR feature-off零成本均成立，WebGPU diagnostics为零。专用镜面地面截图已人工确认反射fixture可见；单帧截图与计数不等价于长时间temporal stability、所有roughness/edge质量或性能结论，后两者继续由扩展视觉与clean formal Gate负责。
