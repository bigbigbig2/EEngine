...
- production filter 顺序已重排为 `raw SSR → TemporalReproject(accumulate=false) → RecurrentDenoise(accumulate=true)`。Temporal 不再先混合receiver/hit velocity后采一个虚构中间坐标，而是分别建立两个物理history candidate：surface candidate从当前receiver center减receiver pixel velocity，并以receiver depth/normal做geometry weight；hit candidate先把当前full-resolution hit texel center映射到effect domain，再减hit pixel velocity，并以当前hit depth/normal校验其4 taps。每个candidate各做一次4-tap geometry-weighted采样，再以dominant ray length/depth得到的hit trust及各自confidence合并；hit texel的motion-invalid或reactive flag会把hit candidate权重清零。motion factor仍按Three的surface/receiver reprojection计算。surface-history UV在任何per-pixel early return之前求`dpdx/dpdy`，由2×2 reprojection Jacobian最小奇异值产生stretch confidence；平方后衰减被放大的history，且stretch/hit trust不足会提高clamp intensity，避免少量旧texel被铺展成拖影。这个结构恢复Three specular temporal的双候选与stretch不变量，但以OEngine velocity field代替Three previous-matrix world-hit projection；half-resolution temporal history fetch从4增至8，必须由本Step尚未执行的PERF Gate验收。3×3 current neighborhood按pinned `color/(1 + luminance×10)`做HDR dampening后统计YCoCg moments；gamma按motion从0.5到1变化，history以AABB center裁剪，再乘原history luminance scale恢复线性HDR。clamp intensity最少0.25并受motion/stretch/hit trust控制，裁剪距离以`exp(-distance×intensity×30)`降低history confidence。shared previous-depth `OcclusionConfidence`、`SurfaceValidity`和current motion-domain geometry继续约束history。history valid、frame index、temporal strength、pre-exposure均execution-time late-bound；compatible history按`current/previous`重标定，且仅在main command成功submit后推进。
