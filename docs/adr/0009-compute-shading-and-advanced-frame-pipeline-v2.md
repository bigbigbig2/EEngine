...
- 双缓冲主 `depth32float` 也只被所有 attachment/sample/copy/HZB source view以 mip0 使用，现从固定5 mip收缩为单 mip；previous-depth双缓冲语义不变，hierarchy仍由独立 `rg16float` HZB完整生产。按几何级数，该收缩相对旧每张5-mip depth allocation理论减少约25%；这只是descriptor/consumer静态事实，实际 bytes 与 GPU 收益仍等待 FrameGraph/浏览器证据。
