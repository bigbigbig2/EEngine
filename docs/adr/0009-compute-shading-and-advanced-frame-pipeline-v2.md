...
- 已用Playwright Chrome启动`surface.post-fusion`，新版浏览器确认暴露WebGPU 2026所需`texture-formats-tier1`，但首次尝试在fixture第0帧被counter capability ledger遗漏阻断，未进入任何Step 9场景阶段；该问题已在Step 10记录中修正，修正后的case尚待重跑。仍未执行TypeScript/build、WGSL compilation、pure tests、source selector、有效SDR/HDR/debug/capture visual、MILESTONE或唯一`comprehensive-full` PERF。因此实际HDR roundtrip减少、variant binding正确性、capture像素语义与性能收益仍是待执行Gate；本记录只表示Step 9 production implementation landed，不表示Exit通过，也不关闭Step 5–8的open验证项。
...
- 最新本地Chrome已暴露`texture-formats-tier1`并进入Renderer初始化，但第一次`surface.post-fusion`尝试在上述capability declaration缺口处于第0帧失败，未形成画面或场景断言证据；该失败直接促成修正，但不能算Step 9 Browser Gate通过。尚未执行type/build、更新后的pure/source oracle、shader audit、Browser MILESTONE或唯一`comprehensive-full` PERF。因此Step 10只能记为“已完成可证明属于此前已过门禁范围的静态cutover cleanup”；Step 4 也已因 corrected Three.js rand 重新打开验证，GTAO/SSGI/SSR/shared products/temporal/post的最终deletion Gate与ADR-0009整体completion仍保持open。
