...
- R3-A 已冻结 multi-instance CPU oracle、Queue ABI、max-cut capacity 和 children 整组 reservation；当前下一步是 R3-B 由 `HierarchicalWorkGenerator` 拥有 frame-local root/ping-pong/selected resources，并接通 GPU Frustum + SSE producer。R3 v1 不直接消费当前独立 BVH8。Packed alpha-tested Visibility、Packed CSM shadow consumer 和 single Material Resolve 分别属于后续 G4-A/G5/G4-B，不由 R3-A 冒充完成。
