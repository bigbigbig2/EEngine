...
- R2-D provenance 清算已补齐 [Packed Visibility](../../references/porting/R2-D-07-packed-visibility.md)、[Material reconstruction](../../references/porting/R2-D-08-packed-material-reconstruction.md) 与 [Velocity](../../references/porting/R2-D-09-packed-velocity.md)；Packed flat loop 已由 R3-D 删除，每材质 fullscreen 仍由 R4-B 替换。
...
- R3 owner 不创建私有 submit/readback；示例中的 queue readback 只用于 GPU/CPU 回归。Packed alpha-tested Visibility、Packed CSM shadow consumer 和 single Material Resolve 分别属于后续 G4-A/G5/G4-B，不由 R3 冒充完成。
