...
- R3-B owner 不创建私有 submit/readback；示例中的 selected queue readback 只用于 GPU/CPU 回归。当前下一步是 R3-C 在同一 GPU 链生成 RasterWork/完整 draw indirect，并由生产 Hardware consumer 直接读取。Packed alpha-tested Visibility、Packed CSM shadow consumer 和 single Material Resolve 分别属于后续 G4-A/G5/G4-B，不由 R3-B 冒充完成。
