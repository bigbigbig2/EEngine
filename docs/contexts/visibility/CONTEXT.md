...
- `R4-A-02` 已让生产 Packed Hardware vertex/fragment 在原有 GPU `RasterWork → drawIndirect` 闭环中写 `VisibilityKey v1 + reverse-Z depth`。Key attachment 由 FrameGraph transient owner 管理，Packed feature-off 不创建；legacy triangle/instance MRT 只为 R4-B 前的旧 consumer 临时保留。sampled reducer 统计 useful/empty/invalid key，submitted fragments 因 WebGPU baseline 无 pipeline statistics 登记为 `WEBGPU-01-PIPELINE-STATISTICS` 平台能力限制。
...
- `R4-A-06` 已用 clean/full A/B/C 关闭 G4-A：同一 production Renderer 保存 final-color oracle、Key heatmap、reverse-Z depth、GPU time、queue/counter 与 diagnostics；三组均一个 main submit/一个 Packed drawIndirect、zero invalid/overflow/WebGPU error，C sampled alpha RasterWork 为 40/127。A/B Hardware Raster P50 约 `35–44 ms`，B 已观察到 `84.260 ms` P99 长尾；这是后续 profile 风险，不阻塞 Hardware contract 正确性关闭。当前入口为 `R4-B-01`。
