...
- `R4-A-06` 已用 clean/full A/B/C 关闭 G4-A：同一 production Renderer 保存 final-color oracle、Key heatmap、reverse-Z depth、GPU time、queue/counter 与 diagnostics；三组均一个 main submit/一个 Packed drawIndirect、zero invalid/overflow/WebGPU error，C sampled alpha RasterWork 为 40/127。A/B Hardware Raster 约 `37–40 ms` 的回退是后续 profile 风险，不阻塞 Hardware contract 正确性关闭。当前入口为 `R4-B-01`。
