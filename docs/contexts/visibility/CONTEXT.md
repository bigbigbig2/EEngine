...
- `R4-A-01` 已将该编码落为内部共享 TS schema/WGSL codec，并冻结 empty/reserved/max、adapter-aware RasterWork capacity、显式 producer failure 与 CPU lookup oracle；multi-Meshlet fixture 已证明两个 RasterWork slot 可唯一定位同一 Cluster 的不同 Meshlet。
- `R4-A-02` 已让生产 Packed Hardware vertex/fragment 在原有 GPU `RasterWork → drawIndirect` 闭环中写 `VisibilityKey v1 + reverse-Z depth`。Key attachment 由 FrameGraph transient owner 管理，Packed feature-off 不创建；legacy triangle/instance MRT 只为 R4-B 前的旧 consumer 临时保留。sampled reducer 统计 useful/empty/invalid key，submitted fragments 因 WebGPU baseline 无 pipeline statistics 登记为 `R4-A-06` blocker。
- `R4-A-03` 已冻结 64 B `MaterialVisibilityRecord v1`，以 4,096 record/256 alpha tile 的有界临时 owner 接入 Packed Scene staging；Geometry GPU ABI v2 在不增加第 9 个 vertex storage binding 的前提下提供 UV0/UV1 fast path。生产单次 drawIndirect 已覆盖 opaque/mask/blend、factor/texture/UV transform/cutoff、double-sided/mirrored 与 invalid texture/sampler fallback；8-slot Chrome gate 零 invalid/depth/WebGPU diagnostics。该 owner 由 `R4-B-02` 接管/删除。
- `R4-A-04` 已将 Packed Visibility debug 接为单个全屏 GPU consumer：从生产 Key 回查同帧 RasterWork/VisibleCluster/Meshlet/Instance/Material，冻结 32 B settings、15 状态和逐层 fail-visible color；debug-off 零 pass/resource/readback/submit。真实 alpha glTF heatmap 与 16-case 生产 WGSL 注入均通过 Chrome 零 diagnostics。
- `R4-A-05` 已把 key/adapter capacity guard 固定在 hierarchy prepare 分配前，并以 GPU completion fence 覆盖 epoch replacement、release 和 abort；resize/camera cut/view recreate/in-flight re-upload/device destroy + fresh Renderer 已通过 production Chrome。counter/debug off 不创建 reducer/readback/额外 submit；256 B disabled sink 仅服务固定 shader binding ABI。当前入口为 `R4-A-06` paired Gate，G4-A 未完成。
