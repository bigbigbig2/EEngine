...
- `R4-A-03` 冻结的 64 B `MaterialVisibilityRecord v1` 已升级为 224 B `MaterialRecord v3`，4,096 record 与 alpha/shading 标准/4K texture residency 由同一 `GpuMaterialVisibilityTable` owner 管理。Geometry GPU ABI v3 提供 UV0/UV1/UV2 fast path；生产单次 drawIndirect 覆盖 opaque/mask/blend、每纹理 UV transform、cutoff、double-sided/mirrored 与 invalid texture/sampler fallback。
