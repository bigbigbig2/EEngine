...
- R3 v1 的 TraversalWork 是 `instanceRecordIndex + clusterRecordIndex`，VisibleCluster 是稳定 lookup seam，RasterWork 是 `visibleClusterSlot + meshletRecordIndex`；当前 R2 BVH8 不直接参与 LOD cut。
