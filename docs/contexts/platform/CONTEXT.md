...
- P2 `RendererConfig` 在初始化前归一化为单一 RenderSettings patch；Renderer 对 adapter 创建和外部传入 Device 使用同一必需 Feature/Limit 校验，缺失时 fail-fast，不进入半兼容路径。每帧 `RenderFrameContract` 只携带冻结标量，不能持有 GPU 对象或触发额外 submit。
