...
- R3-D 已加入 meshoptimizer cone 与 Niagara previous-HZB 保守 reject；mirrored/non-uniform/shear 与 invalid history 的 fail-open 已由 CPU oracle/source gate 冻结。RasterWork expansion 为一个 Cluster 对应一个 64-lane workgroup；`R3-D-08/09` 的 fused-root/workgroup compaction/fused-leaf 已在 clean commit `aff3ab8` 通过 A/B/C，G3 performance 关闭。
