...
- R4-B 必须迁移 R2-D-08/R2-D-09 已验证数学到 Single Resolve，不重新实现另一套 barycentric/gradient/frame/velocity；VisibilityKey 先经 RasterWork 唯一定位 Meshlet。
- Packed Velocity 的 Instance ABI 保存 CPU 预计算 `previous_from_current`；奇异 motion 显式输出零，Shader 禁止逐像素矩阵求逆。
- MaterialTable 与有界 TextureRef/resident handle 是主材质数据源；array-bank、atlas 或 fixed-bank 由真实资产 benchmark 冻结，streaming 不是 v1 Resolve 前提。
- 不按材质数重复全屏扫描。
