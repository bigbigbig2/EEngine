...
- P6 `TransparencyFeature` 现为 Renderer 对 Packed MBOIT 与 legacy Forward/OIT 的唯一 owner；两条路径保持独立中间资源和合成，Packed Reactive/Counter 输出继续供 Temporal 消费，关闭时不创建 OIT 资源。
- P7 `TemporalFeature` 现为 Renderer 对 TAA/TAAU、Temporal Classification、颜色 history、jitter 与 DRS 的唯一 owner；AO/SSR history 继续由各自 Service 独立维护，提交/abort/resize/cut 由共享 Registry 统一失效。
