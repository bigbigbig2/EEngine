...
- 2026-08-30 在 clean commit `74e61c02f3fd66d30bafbf02d8b2472305c9347e` 重采 B/C full：Chrome 151、Intel Gen9、`1280×720`、DPR 1、60 warm-up + 180 sample、每 6 帧 timestamp/counter。两组 artifact/Gate 均通过，issues/counter issues/console/page/validation/uncaptured/device-lost/timestamp/counter diagnostics 全为 0；material sweep 为 `1 → 3` active materials、fullscreen draw 恒为 1。texture lifecycle evidence 为 B `4 resident + 0 retiring + 59 free`、C `0 + 0 + 63`，验证 layer 0 不进入 free-list且采集结束无待退休层。
- 本次 Single Resolve P50/P95/P99 为 B `9.9478445/10.14003285/10.1460405 ms`、C `4.875299/5.04401315/5.08029693 ms`。该结果来自 Intel Gen9，不能与下述 2026-08-28 NVIDIA Turing 数值做性能百分比对比；其作用是为 texture lifecycle、ORM 规范化后的真实 B 资产和新 WGSL 提供 clean browser correctness evidence。artifact 位于 `temp/r4-b/full/`，不纳入 Git。
