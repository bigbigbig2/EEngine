...
- G4-A 只关闭 Hardware key/depth/lookup/alpha/debug/lifecycle contract，不关闭 R4-C SW/Hybrid。R4-A 临时 Material Visibility owner 已由 R4-B 扩展为 128 B `MaterialRecord v2` 与共享 texture residency，不再存在第二套 Packed 材质真相来源。
...
- `R4-B-01..06/09` 已集成，`R4-B-07/08` 按文档条件跳过，Packed `R4-B-10` 已关闭。生产 `PackedMaterialResolvePass` 一次回查 VisibilityKey/RasterWork/VisibleCluster/Instance/Geometry/Meshlet/Material，复用 R2-D-08 analytic gradients/frame 与 R2-D-09 `previous_from_current`，输出 26 B/pixel Surface + velocity；`MaterialRecord v2` 为 128 B，纹理 owner 为 64-layer、`256×256`、9-mip array。
...
1. R4 core、R5 FX-01 与 R5-00 clean/full C baseline 已完成；当前先在干净 commit 上关闭 FX-02 Clustered Direct full Gate。FX-03 尚未开始，FX-02 通过后才进入 IBL Alignment；`performance-targets.json` 与 FX-03 仍阻止 G5-L 关闭。
