...
- Packed Cluster hierarchy/SSE work generation、VisibleCluster/RasterWork、GPU indirect args 与 Hardware consumer；legacy Scene 仍有 flat Meshlet/bucket/scan/expand 路径。
...
- R2 provenance 清算已补齐 Packed glTF 的 Khronos 规范台账与 multi-primitive/material/nested-transform fixture；Packed Material 的 glTF 8/16 位 normalized 与 OEngine 32 位扩展 CPU/WGSL 边界也已冻结。flat Visibility 生产输入已由 R3-C 替换，剩余 paired flat 分支由 R3-D 删除；每材质 fullscreen 仍由 R4-B 替换。
- 生产 Packed Visibility 现执行 InstanceCull → Cluster hierarchy/Frustum/SSE → VisibleCluster → RasterWork → GPU 写完整 16 B indirect record → Hardware `drawIndirect()`，数量不经 CPU readback。旧 flat Meshlet compact 只作为 R3-D 待删的内部 paired reference。Material 每 semantic 只扫描一次 descriptor，使用解析 perspective UV gradient 并修正重复 viewport、镜像/非均匀 normal-tangent；Velocity 已删除每可见像素 `mat4_inverse`。稳定帧不重复 residency/instantiate。
...
- R3-C clean/full flat/hierarchy paired A/B/C 基于 clean commit `0b77ce8`：A 减少 90.1% RasterWork 但 Visibility P50 回退 14.1%；B 减少 80.4% 且 P50 改善 69.6%；C 工作量相同但 hierarchy 多约 0.262 ms 固定成本。六组均无 overflow/WebGPU diagnostics，不能由此宣称 hierarchy 普遍更快。A 的三个主要性能债务是 `workgroup_size(1)` InstanceCull、round 0 和 VisibleCluster expansion。
...
1. R2/G2 与 R3-A/R3-B/R3-C 已完成。当前代码任务是 R3-D：先优化 A 的大规模 workgroup 粒度/queue bandwidth 和 C 的低密度固定成本，再逐项接入 Cone 与 previous HZB，删除 Packed flat producer/queue/indirect owner，重跑 paired A/B/C 后关闭 G3。当前 BVH8 仍不参与 R3 v1 traversal。
2. G3 关闭后依次推进 R4-A Visibility contract、R4-B single Material Resolve、R4-C 可选 SW/Hybrid；Packed alpha-tested Visibility、Packed shadow consumer 与画质管线按各自 Gate 验收，不倒灌回 R2。
