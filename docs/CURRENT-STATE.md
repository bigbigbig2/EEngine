...
- A/B/C 固定 benchmark、instance/meshlet/reject/SW-HW/material 等其余 GPU counter producer、LightCluster overflow bit、统一 debug views 和可用于 gate 的浏览器实机截图/性能 artifact 尚未完成；counter 字段缺失表示 producer 未接入，不能解释为真实零工作量。
