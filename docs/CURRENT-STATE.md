...
- ADR-0010 的完整 lifecycle/overflow 与 paired A/B/C Gate 尚未完成；因此 G4-A 仍未完成。R4-A 临时 Material Visibility owner 是 `R4-B-02` 接管/删除对象，不是第二套长期材质真相来源。
...
1. 执行 `R4-A-05` Overflow、生命周期与 feature-off：补齐 live device-lost、in-flight release、resize 与 capacity failure/fallback 证据。
2. 完成 `R4-A-06` paired 浏览器 Gate 后，才能进入 R4-B single Material Resolve；R4-C SW/Hybrid 继续作为后续 profile optimization。
