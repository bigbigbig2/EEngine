...
shared disocclusion confidence 从当前/上一帧 `depth32float` 与当前 velocity 生成。前两个 binding 必须分别是 depth-only view、WGSL `texture_depth_2d` 和 BGL `sampleType: depth`，第三个才是普通 float velocity texture；4-tap previous-depth gather与3×3 reverse-Z closest-current-depth都直接读取标量。禁止把两张 depth attachment 为了复用颜色 helper而声明成 `texture_2d<f32>`。
