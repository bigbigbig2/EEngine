...
Brick4/Probe raw provider alpha 是 receiver validity；当前迁移实现仍由 scene/frame 级 `indirect_lighting_mode` 选择其中一个 producer，有效 receiver 使用该 producer，无效 receiver 在同一 resolve 回退 IBL。完整的同帧 `valid Brick4 > valid Probe Volume > IBL` receiver chain 尚未落地，现有 counter 只能证明所选 producer/IBL 的互斥归属。SSGI 关闭时 `DiffuseSurfaceLite` 只是已存在 compact Surface 通道的逻辑别名，不产生 SSGI-only attachment；SSGI owner、四张 history、component MRT、trace/filter/resolve 与 evidence dispatch 全部裁剪。
