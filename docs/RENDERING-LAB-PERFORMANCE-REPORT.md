...
9. 材质纹理 residency 增加显式 `RendererConfig.textureMaxResolution`，默认保持原有上限；`?textureMaxResolution=1024` 可在 Rendering Lab 做质量/内存 A/B。静态 1920×1080 full smoke 中，默认配置的材料 resident logical 为 559,248,116 B（约 533.1 MiB），1024 cap 为 139,817,716 B（约 133.4 MiB），25/25 纹理仍驻留，texture/sampler fallback 均为 0，页面无错误；transient pool 两组均约 263.2 MiB，说明纹理 cap 解决的是长期材质 residency，不会掩盖 transient 峰值。两张截图的整体布局和渲染结果可继续用作视觉回归，但 1024 cap 本质是降采样质量 trade-off，不能直接当成无损优化；是否将 cap 设为产品默认，必须在目标显示分辨率和材质近景下做截图/感知质量评审。
