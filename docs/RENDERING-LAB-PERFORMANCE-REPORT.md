...
8. 方向光 CSM 的相机投影重算已增加输入缓存，并用 1e-5 容差匹配矩阵/布局；阴影 raster 仍每帧执行，未改变可见结果，只去掉稳定帧的 frustum slicing 与临时矩阵分配。该阶段同时把 `shadow-update` 纳入 CPU section，并记录 `shadow.directionalCameraUpdates` / `shadow.directionalCameraCacheHits`。同一 1280×720、30+60 full smoke 中，静止段 60/60 帧命中缓存，`shadow-update` P50 约 7.5 ms；运动 path 中 58/60 帧重算、2/60 命中，说明相机变化仍会使缓存失效。静止段相对未加容差的探索结果（P50 约 7.7 ms）只下降约 0.2 ms，低于单机短跑噪声，不能宣称固定收益；后续高收益方向仍是基于 caster/灯光 dirty state 的 shadow raster 更新降频，而不是继续放宽矩阵容差。
