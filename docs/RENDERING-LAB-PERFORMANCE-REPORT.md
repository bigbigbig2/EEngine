...
8. 方向光 CSM 的相机投影重算已增加输入缓存，并用 1e-5 容差匹配矩阵/布局；同时增加 scene change revision + Packed GPU epoch 的 raster dirty gate。阴影 raster 只有投影变化、场景/实例内容变化或首次渲染时提交，静态帧跳过重复的 CSM 编码；动态 patch 会把当前帧显式标脏，避免 stale shadow。该阶段把 `shadow-update` 纳入 CPU section，并记录 `shadow.directionalCameraUpdates` / `shadow.directionalCameraCacheHits` / `shadow.directionalRasterDraws` / `shadow.directionalRasterSkips`。新增 `animateScene:false` 基准开关后，同一 1280×720、30+60 full smoke 的静止 workload 在 measured window 中为 `cameraHits=60/60`、`rasterSkips=60/60`，`shadow-update` P50 约 0.1 ms；静态场景+运动 path 为 `cameraUpdates=58/60`、`rasterDraws=58/60`，相机变动会正确使缓存失效。动态 workload 仍每帧注入 Packed patch，因此不会错误跳过阴影。该结果证明 dirty gate 的行为正确，但 CPU/GPU 的最终收益仍需在目标硬件和更长 paired run 上确认。
9. 材质纹理 residency 增加显式 `RendererConfig.textureMaxResolution`，默认保持原有上限；`?textureMaxResolution=1024` 可在 Rendering Lab 做质量/内存 A/B。静态 1920×1080 full smoke 中，默认配置的材料 resident logical 为 559,248,116 B（约 533.1 MiB），1024 cap 为 139,817,716 B（约 133.4 MiB），25/25 纹理仍驻留，texture/sampler fallback 均为 0，页面无错误；transient pool 两组均为 263,174,559 B（约 251.0 MiB），说明纹理 cap 解决的是长期材质 residency，不会掩盖 transient 峰值。两张截图的整体布局和渲染结果可继续用作视觉回归，但 1024 cap 本质是降采样质量 trade-off，不能直接当成无损优化；是否将 cap 设为产品默认，必须在目标显示分辨率和材质近景下做截图/感知质量评审。
