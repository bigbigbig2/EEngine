...
const controller=createValidationController({caseId:"sparse-shading-candidate",workloadId:"sparse-shading-candidate-correctness-v6"},async()=>{
    ...
    if(adapter===null)controller.unsupported("No WebGPU core adapter is available");
      ...
      if(plan!==undefined){
        ...
        const capability=captureGpuSparseShadingCapabilityRecord(plan,{features:device.features,limits:snapshotGpuLimits(device.limits),
          ...
          formatProfile:"adr-0013-step-6-correctness-l4-v6"});
        ...
        const unlitVertexColor=await fixture.runUnlitVertexColor();fixture.destroy();const unlitVertexColorDisposal=fixture.resourceCounts();
        fixture=await SparseShadingCandidateFixture.create(device,context,format,capability,"unlit-texture");
        const unlitTexture=await fixture.runUnlitTexture();fixture.destroy();const unlitTextureDisposal=fixture.resourceCounts();
        fixture=await SparseShadingCandidateFixture.create(device,context,format,capability,"rendering-lab-fixed");
        const renderingLabFixed=await fixture.runRenderingLabFixed();fixture.destroy();
        const renderingLabDisposal=fixture.resourceCounts();
        fixture=await SparseShadingCandidateFixture.create(device,context,format,capability,"lifecycle-resize");
        const resizeLifecycle=await fixture.runResizeLifecycle();controller.transition("draining");
        controller.addEvidence("readback",{schemaVersion:6,phase:"candidate-pipeline-gpu-execution-oracle",
          scenarios:{mixedBins,basicCube,unlitVertexColor,unlitTexture,renderingLabFixed,resizeLifecycle},
          intermediateDisposal:{mixedBins:mixedBinsDisposal,basicCube:basicCubeDisposal,unlitVertexColor:unlitVertexColorDisposal,
            unlitTexture:unlitTextureDisposal,renderingLabFixed:renderingLabDisposal}});
        controller.addEvidence("submit",{main:11,private:0});if(status)status.textContent=
          "passed ADR-0013 Step 6 candidate correctness and resize lifecycle workloads";
        controller.pass();
      }
    }
  }
